...
`).trim(),za(a)?"":a.includes("WHAT HAPPENED")||a.includes("KEY POINTS")?a:Fa(a)},ue=(e,a)=>Lt(a||"")||Lt(e.summary)||e.headline,ja=e=>{let r=e?.closest(".detail-summary, .thread-item, .market-thread-update")?.querySelector(".briefing-capture-area");if(!r||!window.html2canvas)return;let o=document.createElement("div");o.className="briefing-export-staging";let s=r.cloneNode(!0);s.classList.add("briefing-export-surface"),o.appendChild(s),document.body.appendChild(o);let n=document.documentElement.dataset.theme==="light";window.html2canvas(s,{backgroundColor:n?"#f5f7fb":"#0f172a",scale:2,logging:!1,useCORS:!0}).then(i=>{let l=document.createElement("a");l.download="siftle-briefing.png",l.href=i.toDataURL("image/png"),l.click(),window.showActionToast?.("Briefing card image downloaded!")}).catch(()=>{window.showActionToast?.("Unable to download briefing card")}).finally(()=>{o.remove()})};window.downloadBriefingCard=ja;var Wa=e=>e.toString().toLowerCase().trim().replace(/\s+/g,"-").replace(/[^\w\-]+/g,"").replace(/\-\-+/g,"-").replace(/^-+/,"").replace(/-+$/,""),qa=(e,a)=>{let r="";if(a)try{r=decodeURIComponent(a)}catch{r=a}let o=window.location.origin,s=window.location.pathname,n=t.stories.find(c=>c.id===e||r&&c.sourceUrl===r),i=n?Wa(n.headline):e>0?`story-${e}`:"",l=e>0?`${o}/story/${i}?utm_source=briefing&url=${encodeURIComponent(n?.sourceUrl||r)}`:r?`${o}/api/redirect?url=${encodeURIComponent(r)}&source=briefing`:`${o}/story/briefing?utm_source=briefing`;navigator.clipboard.writeText(l).then(()=>{w("Shareable link copied to clipboard!")}).catch(()=>{w("Unable to copy link")})};window.copyBriefingLink=qa;var ft=(e,a)=>{let r=e.split(/(?:\*\*|__)?(WHAT HAPPENED|KEY POINTS|TAKEAWAY)\s*:?\s*(?:\*\*|__)?\s*:?\s*/i);if(r.length<=1)return`<p class="briefing-text">${e}</p>`;let o='<div class="briefing-capture-area">';if(a){let n=a.headline||"Football Match Update";o+=`
    ...
    `}r[0].trim()&&(o+=`<p class="briefing-capture-intro">${r[0].trim()}</p>`);let s="";for(let n=1;n<r.length;n+=2){let i=r[n].trim().toUpperCase(),l=r[n+1]?r[n+1].trim():"";if(!l)continue;let c="";if(i==="KEY POINTS"){let u=l.split(/(?:•|\*|-)\s+/).map(d=>d.replace(/\\n/g,"").trim()).filter(d=>{if(!d||d==="\\n"||d===`
...
`,Sr=(e=4)=>`${ke("Loading stories")}${Array.from({length:e},kr).join("")}`,ye=()=>`
...
`,ot=e=>{let a=e.type==="tweet",r='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: text-top; color: var(--color-text-primary);"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',o='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 12px; height: 12px; display: inline-block; vertical-align: text-top; margin-right: 4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';return`
  ...
  `};var we=()=>{if(!S)return;if(S.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){S.innerHTML=Sr(4);return}if(te){S.innerHTML=jr(),ie();return}let e=T(t.newsSearchQuery.trim()),r=`
...
Trade and discuss here: ${l}`},Nr=e=>{if(!S||!L)return;let a=Je(e),r=!t.checkedMarketEvidence[e.id],o=K(e),s=t.marketSnapshots[e.id],n=oe(e),i=ne(e);n&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let l=nr(e),c=!!(o&&!s),p=s?.yesPriceCents??(o?e.probability:0),u=s?.noPriceCents??(o?100-e.probability:0),d=c?"":o?`${p}\xA2`:"--",g=c?"":o?`${u}\xA2`:"--",v=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},y=!!v.optionId;n&&y&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),n&&!y&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let x=n&&t.marketOrderMode==="sell"&&y?Math.max(0,Number(v.optionSharesUsdc)||0):0,k=x>0?x:De(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,v),m=x>0?{min:0,max:x}:na(t.marketOrderMode,t.marketTradeSide,v),f=t.marketOrderMode==="buy"?"exactly $2.00 USDC":`Up to $${H(m.max)} USDC`,A=!t.walletAddress||t.hasLoadedPortfolioPositions,b=Ke(e,s),M=Qt(e,s),$=!!M;n||(t.marketTradeSide=wt(t.marketOrderMode,t.marketTradeSide,v));let P=!n&&!b&&!$&&A&&de(t.marketOrderMode,"yes",v),N=!n&&!b&&!$&&A&&de(t.marketOrderMode,"no",v),Y=n?!b&&!$&&A&&(t.marketOrderMode==="sell"?y:!y&&!!l):!b&&!$&&A&&de(t.marketOrderMode,t.marketTradeSide,v),q=b?"Market resolved":M||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),V=b?"Market resolved":M||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),Z=n?k:ia(s,t.marketTradeSide,k,t.marketOrderMode,v),Q=t.marketOrderMode==="buy"?"Buy":"Exit",ge=n?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";S.hidden=!0,L.hidden=!1,L.classList.add("fullscreen"),document.body.classList.add("detail-mode"),gr(e),oa(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&z({force:!t.hasLoadedPortfolioPositions});let St=n?!!v.optionId:v.yesSharesUsdc>0||v.noSharesUsdc>0,Ze="";n&&St&&t.walletAddress?Ze=`
  ...
  `,((m="2026-07-19T23:59:59.000Z")=>{let f=document.getElementById("seasonTimer");ee&&clearInterval(ee);let A=()=>{let M=new Date(m).getTime()-new Date().getTime();if(M<=0){f&&(f.innerText="Season Finished!"),ee&&clearInterval(ee);return}let $=Math.floor(M/(1e3*60*60*24)),P=Math.floor(M%(1e3*60*60*24)/(1e3*60*60)),N=Math.floor(M%(1e3*60*60)/(1e3*60)),Y=Math.floor(M%(1e3*60)/1e3);f&&(f.innerText=`${$}d ${P}h ${N}m ${Y}s`)};A(),ee=setInterval(A,1e3)})();let r=m=>m.map((f,A)=>{let b=Number(f.globalRank)||A+1,M=String(f.username||""),$=!!(t.walletAddress&&M.toLowerCase()===t.walletAddress.toLowerCase()),P=$&&t.profileUsername?t.profileUsername:f.displayName||M,N=$?`${t.profileUsername?P:R(M)} (You)`:P.startsWith("0x")&&P.length===42?R(P):P,Y=T(N),q=Ut(f.status);if(f.totalTrades!==void 0&&f.aiBriefingUnlocks!==void 0){let Ve=Ct(f.status);q=`${Ve.wins}W - ${Ve.losses}L \xB7 ${f.totalTrades} trades \xB7 ${f.aiBriefingUnlocks} unlocks`}let V=T(q),Z=f.nextSeasonDivision?`Division ${f.nextSeasonDivision}`:"Qualify",Q=b<=10?"promotion-zone":"safety-zone",ge=b<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
    ...
    `}let o=Or(r?.outcome),s=la(a,r),n=s.reduce((y,x)=>Math.max(y,x.payout),0),i=a.yesSharesUsdc+a.noSharesUsdc,l=r?.outcome??0,c=qe().has(e.id),p=l===1?a.yesSharesUsdc:l===2?a.noSharesUsdc:0,u=l===1?r?.yesSharesUsdc??0:l===2?r?.noSharesUsdc??0:0,d=r?.volumeUsdc??0,g=p>0&&u>0?p/u*d:0,v=l===0?"":c?'<span style="color: #34d399; font-size: 0.82rem; font-weight: 800;">Claimed</span>':g>0?`<button type="button" class="connect-wallet-btn" data-claim-market="${e.id}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 6px !important; padding: 8px 14px !important; font-size: 0.82rem !important; font-weight: 700 !important; cursor: pointer !important;">Claim $${H(g)}</button>`:'<span style="color: #ef4444; font-size: 0.82rem; font-weight: 800;">Lost</span>';return`
    <article class="portfolio-position-card">
      <div class="portfolio-position-top">
        <span class="category-chip ${e.category}">${W(e.category)}</span>
        <span>${o}</span>
  ...
  `},Hr=async e=>{if(!t.walletAddress){w("Please sign in first.");return}let a=Le().find(o=>o.id===e),r=a?K(a):"";if(!a||!r){w("Market is not available.");return}try{t.claimingMarketIds[a.id]=!0,F(),U("claim_attempt"),kt();let o=await xa(r,t.walletAddress);U("claim_success"),o.won&&cr(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ce(t.walletAddress),await z(),w(o.won?`Claimed $${H(o.amountUsdc)}`:"No payout to claim"),_(),F()}catch(o){U("claim_failed"),w(o instanceof Error?o.message:"Claim failed")}finally{delete t.claimingMarketIds[a.id],F()}},Rr=e=>{if(!e)return"";let a=t.referralData,r=a?.referrals?.length?a.referrals.map(s=>{let n=s.displayName||R(s.walletAddress),i=s.remaining<=0;return`
  ...
  `},h=()=>{if(Yt.forEach(e=>{let a=e.dataset.bottomNav;e.classList.toggle("active",a==="saved"?t.showSaved:a===t.activeSurface&&!t.showSaved)}),t.activeSurface==="markets"){ma();return}if(t.activeSurface==="portfolio"){F();return}if(t.activeSurface==="leaderboard"){fa();return}ga(),ta(),we(),Br(),j&&(j.value=t.activeArchiveDate??"")};nt.textContent=Jt();ae?.addEventListener("click",e=>{let r=e.target.closest("[data-category]");if(!r)return;let o=r.dataset.category;t.activeCategory=o,te=!1,re=null,window.history.pushState({},"","#feed"),J(),h(),o==="Personalized"&&!ha()&&Fe(),_e(),se(t.activeCategory)});var te=!1,Nt="overall",re=null,ze=!1,Ne=()=>{try{let e=localStorage.getItem("siftle_followed_entities");if(e)return JSON.parse(e)}catch{}return{clubs:[],managers:[],players:[]}},ha=()=>{let e=Ne();return(e.clubs?.length||0)+(e.managers?.length||0)+(e.players?.length||0)>0},zr=e=>{localStorage.setItem("siftle_followed_entities",JSON.stringify(e))},Fr=e=>{if(!e)return"";let a=e.replace(/\r\n/g,`
`).replace(/\r/g,`
`),o=pt(a).split(`
`),s="",n="",i=!1,l=!1;for(let c=0;c<o.length;c++){let p=o[c].trim();if(!p)continue;if(/what matters/i.test(p)||p.includes("\u{1F3AF}")){i&&(n+="</ul></div>",i=!1);let g=p.replace(/^#+\s*/,"").replace(/[🎯⚡⏱️⭐]\s*/g,"");n+=`
  ...
  `},ie=()=>{document.querySelector("#backToFeedBtn")?.addEventListener("click",()=>{te=!1,h()}),document.querySelector("#openBriefingBtn")?.addEventListener("click",()=>{te=!0;let e=t.activeCategory==="Personalized"?"personalized":"overall";Nt!==e&&(re=null),Nt=e,h(),re||ut(!1)}),document.querySelector("#catchUpAgainBtn")?.addEventListener("click",()=>{ut(!1)})},ut=async(e=!1)=>{ze=!0,te&&h();let a=e?null:localStorage.getItem("siftle_last_briefing_at"),r=Ne();try{let o=await fetch(I("/api/briefing/delta"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lastBriefingAt:a,context:t.activeCategory==="Personalized"?"personalized":"overall",entities:r})}),s=await o.json();ze=!1,o.ok&&s.success?(re=s,localStorage.setItem("siftle_last_briefing_at",s.periodEnd||new Date().toISOString()),te&&h()):(re={periodStart:new Date().toISOString(),markdown:`### Failed to generate briefing

${s.error||"Please try again in a moment."}`},te&&h())}catch(o){ze=!1,re={periodStart:new Date().toISOString(),markdown:`### Failed to connect to briefing service

${o.message}`},te&&h()}};S?.addEventListener("input",e=>{let a=e.target;if(a.id!=="newsSearchInput")return;let r=a.selectionStart??a.value.length,o=a.selectionEnd??a.value.length;t.newsSearchQuery=a.value,we();let s=S?.querySelector("#newsSearchInput");s&&(s.focus(),s.setSelectionRange(r,o))});Pe?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="markets",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,window.history.pushState({},"","#markets"),J(),h()});Ue?.addEventListener("click",()=>{t.activeSurface="feed",t.showSaved=!1,window.history.pushState({},"","#feed"),J(),h(),_e(),se(t.activeCategory)});Ce?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="portfolio",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=!1,window.history.pushState({},"","#portfolio"),J(),h()});le?.addEventListener("click",()=>{t.walletAddress?(window.location.hash="#portfolio",Be()):Ie()});document.addEventListener("click",e=>{let a=e.target,r=a.closest(".copy-address-btn");if(r){let i=r.getAttribute("data-address");i&&navigator.clipboard.writeText(i).then(()=>{w("Wallet address copied!")})}let o=a.closest("[data-claim-market]");if(o){let i=o.getAttribute("data-claim-market");i&&Hr(i);return}if(a.closest("[data-open-referrals]")){t.referralPanelOpen=!t.referralPanelOpen,!t.referralData&&!t.loadingReferralData&&Me(),F();return}if(a.closest("[data-close-referrals]")){t.referralPanelOpen=!1,F();return}if(a.closest("[data-refresh-referrals]")){t.referralError=null,Me(),F();return}let s=a.closest("[data-copy-referral-code]");if(s){let i=s.getAttribute("data-copy-referral-code")||"";i&&navigator.clipboard.writeText(i).then(()=>w("Invite code copied"));return}let n=a.closest("[data-copy-referral-link]");if(n){let i=n.getAttribute("data-copy-referral-link")||"";i&&navigator.clipboard.writeText(i).then(()=>w("Invite link copied"));return}a.closest("[data-connect-wallet]")&&(t.walletAddress?Rt():Ie())});Yt.forEach(e=>{e.addEventListener("click",()=>{let a=e.dataset.bottomNav;t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=a==="saved",a==="markets"?(t.activeSurface="markets",window.history.pushState({},"","#markets")):a==="portfolio"?(t.activeSurface="portfolio",window.history.pushState({},"","#portfolio")):a==="leaderboard"?(t.activeSurface="leaderboard",window.history.pushState({},"","#leaderboard")):(t.activeSurface="feed",window.history.pushState({},"","#feed"),_e(),se(t.activeCategory),a==="saved"&&(lr(),mt(),Te())),J(),h()})});j?.addEventListener("change",()=>{t.activeArchiveDate=j.value||null,window.history.pushState({},"","#feed"),J(),h(),se(t.activeCategory)});_a?.addEventListener("click",()=>{t.activeArchiveDate=null,j&&(j.value=""),window.history.pushState({},"","#feed"),J(),h(),se(t.activeCategory)});S?.addEventListener("click",async e=>{let a=e.target;if(a.closest("#editUsernameBtn")){let k=S?.querySelector(".username-display-row"),m=S?.querySelector("#usernameEditForm");if(k&&m){k.style.display="none",m.style.display="flex";let f=m.querySelector("#usernameInput");f&&f.focus()}return}if(a.closest("#cancelUsernameBtn")){let k=S?.querySelector(".username-display-row"),m=S?.querySelector("#usernameEditForm");k&&m&&(k.style.display="flex",m.style.display="none");return}let s=a.closest("#saveUsernameBtn");if(s){let m=S?.querySelector("#usernameEditForm")?.querySelector("#usernameInput");if(m){let f=m.value.trim().slice(0,15),A=s,b=A.textContent||"Save";A.disabled=!0,A.textContent="Saving...",ir(f),t.profileNotice=null;try{t.walletAddress&&await fe(!1),t.profileNotice={type:"success",message:"Username saved to your shared leaderboard profile."},w("Username updated"),F()}catch(M){let $=M instanceof Error?M.message:"Username save failed";t.profileNotice={type:"error",message:$},w($),A.disabled=!1,A.textContent=b,F()}}return}let n=a.closest("[data-portfolio-filter]");if(n){let k=n.getAttribute("data-portfolio-filter");t.portfolioFilter=k,F();return}let i=a.closest("[data-timeframe]");if(i){let k=i.dataset.timeframe;t.activeMarketTimeframe=k,ma();return}let l=a.closest("[data-market-id]");if(l){t.selectedMarketId=l.dataset.marketId??null,U("market_view"),window.history.pushState({},"",`#market-${t.selectedMarketId}`),h(),window.scrollTo({top:0,behavior:"smooth"});return}if(a.closest(".read-tweet-btn")){e.stopPropagation();let k=a.closest("[data-story-id]");k&&lt(Number(k.dataset.storyId),!0);return}let p=a.closest("[data-thread-story-id]"),u=a.closest("[data-export-id]"),d=a.closest("[data-export-action]"),g=a.closest("[data-story-id]");if(p){e.stopPropagation();let k=t.stories.find(m=>m.id===Number(p.dataset.threadStoryId));k&&Va(k);return}let v=a.closest(".mobile-bookmark-btn, .bookmark-button");if(v){e.stopPropagation();let k=v.dataset.bookmarkUrl||"",m=t.stories.find(f=>f.sourceUrl===k);if(!m)return;m.saved=!m.saved,m.saved?pe.add(k):pe.delete(k),Ba(),w(m.saved?"Saved to your list":"Removed from saved"),we();return}if(d){e.stopPropagation(),Cr(Number(d.dataset.exportStoryId),d.dataset.exportAction);return}if(u){e.stopPropagation();let k=Number(u.dataset.exportId);t.activeShareStoryId=t.activeShareStoryId===k?null:k,we();return}if(!g||a.closest("a"))return;let y=Number(g.dataset.storyId),x=t.stories.find(k=>k.id===y);x&&U("feed_story_click",x.sourceUrl,x.headline),lt(y,!0)});S?.addEventListener("keydown",e=>{let r=e.target.closest("[data-story-id]");if(!r||e.key!=="Enter"&&e.key!==" ")return;e.preventDefault();let o=Number(r.dataset.storyId),s=t.stories.find(n=>n.id===o);s&&U("feed_story_click",s.sourceUrl,s.headline),lt(o)});L?.addEventListener("click",e=>{let a=e.target,r=a.closest("[data-unlock-briefing]");if(r){let u=t.stories.find(d=>d.id===Number(r.dataset.unlockBriefing));u&&it(u);return}let o=a.closest("[data-unlock-briefing-url]");if(o){let u=decodeURIComponent(o.dataset.unlockBriefingUrl||""),d=Ja(u);d&&(me(d)?Ae(d):it(d));return}if(a.closest("[data-back-markets]")){t.selectedMarketId=null,t.tradeDrawerOpen=!1,window.history.pushState({},"","#markets"),h();return}if(a.closest("#openTradeDrawerBtn")){let u=E.find(v=>v.id===t.selectedMarketId);if(u){if(Ke(u,t.marketSnapshots[u.id])){w("This market is resolved and can no longer be traded.");return}if(Qt(u,t.marketSnapshots[u.id])){w("Trading is locked 20 minutes before kickoff.");return}}t.tradeDrawerOpen=!0,U("trade_drawer_open");let d=L.querySelector("#tradeDrawer"),g=L.querySelector("#tradeDrawerBackdrop");d?.classList.add("open"),g?.classList.add("open");return}if(a.closest("#closeTradeDrawerBtn")||a.closest("#tradeDrawerBackdrop")){t.tradeDrawerOpen=!1;let u=L.querySelector("#tradeDrawer"),d=L.querySelector("#tradeDrawerBackdrop");u?.classList.remove("open"),d?.classList.remove("open");return}if(a.closest("#shareWhatsAppBtn")){let u=E.find(d=>d.id===t.selectedMarketId);if(u){let d=Dr(u),g=`https://api.whatsapp.com/send?text=${encodeURIComponent(d)}`;window.open(g,"_blank")}return}let s=a.closest("[data-market-trade]");if(s&&t.selectedMarketId){let u=s.dataset.marketTrade;hr(t.selectedMarketId,u);return}let n=a.closest("[data-market-option-trade]");if(n&&t.selectedMarketId){let u=n.dataset.marketOptionTrade||t.marketTradeOptionId||"";Ir(t.selectedMarketId,u);return}let i=a.closest("[data-market-option-id]");if(i){if(i.disabled||i.classList.contains("disabled"))return;t.marketTradeOptionId=i.dataset.marketOptionId||null,h();return}let l=a.closest("[data-market-trade-side]");if(l){if(l.disabled||l.classList.contains("disabled"))return;let u=E.find(v=>v.id===t.selectedMarketId),d=u?t.marketPositions[u.id]:void 0,g=l.dataset.marketTradeSide;if(!de(t.marketOrderMode,g,d))return;t.marketTradeSide=g,h();return}let c=a.closest("[data-market-order-mode]");if(c){t.marketOrderMode=c.dataset.marketOrderMode;let u=E.find(g=>g.id===t.selectedMarketId),d=u?t.marketPositions[u.id]:void 0;t.marketTradeSide=wt(t.marketOrderMode,t.marketTradeSide,d),t.marketTradeAmount=De(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,d),h();return}let p=a.closest("[data-back-to-feed]");if(p){if(p.classList.contains("read-more-news-btn")){let d=new URLSearchParams(window.location.search).get("url"),g=document.querySelector(".detail-card h2")?.textContent||void 0;U("shared_read_more_click",d||void 0,g)}Za()}});L?.addEventListener("input",e=>{let a=e.target;if(!a.matches("[data-market-amount]"))return;let r=E.find(c=>c.id===t.selectedMarketId),o=r?t.marketSnapshots[r.id]:void 0,s=r?t.marketPositions[r.id]:void 0,n=Number(a.value);t.marketTradeAmount=Number.isFinite(n)?n:0;let i=r&&oe(r)?t.marketTradeAmount:ia(o,t.marketTradeSide,t.marketTradeAmount,t.marketOrderMode,s),l=L.querySelector(".market-inline-payout strong");l&&(l.textContent=`$${H(i)}`)});L?.addEventListener("focusin",e=>{e.target.matches("[data-market-amount]")&&document.body.classList.add("market-amount-focused")});L?.addEventListener("focusout",e=>{let a=e.target;if(a.matches("[data-market-amount]")){let r=E.find(s=>s.id===t.selectedMarketId),o=r?t.marketPositions[r.id]:void 0;t.marketTradeAmount=De(Number(a.value)||0,t.marketOrderMode,t.marketTradeSide,o),a.value=String(t.marketTradeAmount),window.setTimeout(()=>document.body.classList.remove("market-amount-focused"),120)}});window.addEventListener("popstate",Be);window.addEventListener("hashchange",Be);window.addEventListener("focus",async()=>{if(t.walletAddress){let e=t.walletBalance,a=await ce(t.walletAddress);t.walletBalance=a,_(),(!e||parseFloat(e)===0)&&parseFloat(a)>0&&(console.log("[X402] Balance changed from 0 to positive. Triggering Gateway warmup..."),Ma())}});Xe?.addEventListener("click",()=>{if(!et||!Xe)return;let e=!et.hidden;et.hidden=e,Xe.setAttribute("aria-expanded",String(!e))});document.addEventListener("click",e=>{let a=e.target;!a.closest(".share-control")&&t.activeShareStoryId!==null&&(t.activeShareStoryId=null,we());let r=a.closest("[data-menu-action]");if(!r)return;let o={today:"Showing today's brief",saved:`${t.stories.filter(s=>s.saved).length} saved stories`,archive:t.archiveDates.length>0?"Choose a saved day from the archive selector":"No saved days yet"};C&&(C.textContent=o[r.dataset.menuAction??"today"]),r.dataset.menuAction==="today"&&(t.showSaved=!1,t.activeArchiveDate=null,j&&(j.value=""),J(),_e(),se(t.activeCategory)),r.dataset.menuAction==="saved"&&(ga(),mt(),Te(),t.showSaved=!0,document.querySelector("#archiveControls")?.classList.remove("mobile-open"),J(),h())});var Wr=async()=>{try{let e=await fetch(I("/api/summary/unlock-config"));e.ok&&(t.unlockConfig=await e.json(),h())}catch(e){console.error("Failed to prefetch unlock config:",e)}},qr=()=>{window.setInterval(async()=>{try{let e=new AbortController,a=window.setTimeout(()=>e.abort(),3500),r=await fetch(I("/api/markets"),{signal:e.signal});if(window.clearTimeout(a),r.ok){let o=await r.json();Array.isArray(o)&&o.length>0&&(E=o,t.activeSurface==="markets"&&h())}}catch(e){console.warn("Background market poll failed:",e)}},3e4)};h();_();Wr();se(t.activeCategory);qr();Ea().then(()=>{fr(),h(),_(),window.setTimeout(Jr,1200),er()});var Yr=document.querySelector("#mobileArchiveCard"),be=document.querySelector("#archiveControls");Yr?.addEventListener("click",()=>{if(!be)return;be.classList.toggle("mobile-open")&&setTimeout(()=>be.scrollIntoView({behavior:"smooth",block:"center"}),50)});var Gr=document.querySelector("#archivePill");Gr?.addEventListener("click",e=>{if(e.stopPropagation(),!be)return;be.classList.toggle("mobile-open")&&setTimeout(()=>be.scrollIntoView({behavior:"smooth",block:"center"}),50)});var He=!1,Ot=!1,Jr=()=>{Ot||(Ot=!0,(async()=>{let e=await je();if(He=!!e,e){t.walletConnecting=!0,_();try{let a=await $a();He=!1,t.walletConnecting=!1,a?(t.walletAddress=await je(),t.walletAddress&&(ve(),t.walletBalance=await ce(t.walletAddress),await z()),_(),t.activeSurface==="portfolio"&&h()):(t.walletAddress=null,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ve(),w("Session expired. Please sign in again."),_(),h())}catch(a){console.warn(a),He=!1,t.walletConnecting=!1,t.walletAddress=null,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ve(),w("Session expired. Please sign in again."),_(),h()}}await Ta(a=>{He||(t.walletAddress=a,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ve(),a&&fe(!1).catch(r=>console.error("Failed to report leaderboard entry:",r)),t.marketPositions={},t.hasLoadedPortfolioPositions=!1,_(),a?(Me(),ce(a).then(r=>{t.walletBalance=r,_(),t.activeSurface==="portfolio"&&h()}),z()):t.activeSurface==="portfolio"&&h())})})())};U("app_open");document.addEventListener("click",e=>{let r=e.target.closest("a, button");if(r){let o=r.className||"",s=typeof o=="string"?o:r.getAttribute("class")||"",n=r.getAttribute("href")||"";!(r.hasAttribute("data-unlock-briefing")||r.hasAttribute("data-unlock-briefing-url")||r.classList.contains("summary-btn")||r.textContent?.trim()==="AI briefing"||r.textContent?.trim().includes("Unlock via"))&&(s.includes("source-button")||s.includes("source-btn")||s.includes("source-link")||r.textContent?.trim()==="Open source")&&!s.includes("disabled")&&n!=="#"&&U("open_source")}},!0);

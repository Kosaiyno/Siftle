...
`,pe=()=>{if(!b)return;let e=oa();if(b.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){b.innerHTML=Ua(4);return}if(e.length===0){let a=t.showSaved?[]:t.stories;if(a.length>0){b.innerHTML=a.map(r=>`
        <article class="story-card" data-story-id="${r.id}" role="button" tabindex="0" aria-label="Open summary for ${r.headline}">
          ...
          <div class="story-copy desktop-only">
            <span class="category-chip ${r.category}">${D(r.category)}</span>
            <h2>${r.headline}</h2>
      ...
      `).join("");return}b.innerHTML="";return}b.innerHTML=e.map(a=>`
        <article class="story-card" data-story-id="${a.id}" role="button" tabindex="0" aria-label="Open summary for ${a.headline}">
          ...
          <div class="story-copy desktop-only">
            <span class="category-chip ${a.category}">${D(a.category)}</span>
            <h2>${a.headline}</h2>
            <p>Tap to read the AI briefing.</p>
          </div>
  ...
  <article class="thread-item">
    ...
    <div class="thread-item-body">
      ...
      ${ee(e)?t.loadingSummaryUrl===e.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${je(Y(e,t.aiSummaries[e.sourceUrl]||e.ai_summary))}</div>`:""}
    </div>
  </article>
...
Trade and discuss here: ${m}`},ja=e=>{if(!b||!k)return;let a=Te(e),r=!t.checkedMarketEvidence[e.id],o=j(e),s=t.marketSnapshots[e.id],i=!!(o&&!s),n=s?.yesPriceCents??(o?e.probability:0),m=s?.noPriceCents??(o?100-e.probability:0),c=i?"":o?`${n}\xA2`:"--",l=i?"":o?`${m}\xA2`:"--",g=ze(Number(t.marketTradeAmount)||0),d=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},u=!t.walletAddress||t.hasLoadedPortfolioPositions,p=Ye(e,s),f=xt(e,s),v=!!f;t.marketTradeSide=Ge(t.marketOrderMode,t.marketTradeSide,d);let $=!p&&!v&&u&&Z(t.marketOrderMode,"yes",d),x=!p&&!v&&u&&Z(t.marketOrderMode,"no",d),A=!p&&!v&&u&&Z(t.marketOrderMode,t.marketTradeSide,d),_=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),L=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),W=Et(s,t.marketTradeSide,g,t.marketOrderMode,d),B=t.marketOrderMode==="buy"?"Buy":"Exit",ae=o?"Arc testnet live":"Contract not deployed";b.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),Aa(e),Ut(e),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let U=d.yesSharesUsdc>0||d.noSharesUsdc>0,G="";U&&t.walletAddress&&(G=`
                ...
                <article class="market-thread-update">
                  ...
                  <div class="market-thread-update-content">
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
                      ...
                      <button type="button" class="market-thread-source-link" data-unlock-briefing-url="${encodeURIComponent(S.sourceUrl)}">${ee(we(e,S))?"AI briefing":"Unlock AI briefing"}</button>
                    </div>
                    ${ee(we(e,S))?t.loadingSummaryUrl===S.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${je(Y(we(e,S),t.aiSummaries[S.sourceUrl]))}</div>`:""}
                  </div>
                </article>
              `).join("")}
            </div>
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${i?`
              ...
              `:`
                <button type="button" class="market-side yes ${t.marketTradeSide==="yes"?"active":""} ${$?"":"disabled"}" data-market-trade-side="yes" ${$?"":"disabled"} title="${$?"Yes":_}">
                  <span>Yes</span>
                  <strong>${c}</strong>
                  ${$?"":`<small>${_}</small>`}
                </button>
  ...
  `},_t=()=>{if(!b||!k)return;Se?.toggleAttribute("hidden",!0),xe?.toggleAttribute("hidden",!0),X?.toggleAttribute("hidden",!0),he?.classList.remove("active"),ge?.classList.remove("active"),ve?.classList.remove("active"),document.body.classList.remove("detail-mode"),k.hidden=!0,b.hidden=!1,b.classList.add("markets-list"),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let e=t.walletAddress&&t.hasLoadedPortfolioPositions?Je():null;t.walletAddress&&e&&fetch(E("/api/leaderboard/report"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({walletAddress:t.walletAddress,points:e.points,status:e.status,username:t.profileUsername||""})}).catch(d=>console.error("Failed to report user score:",d)),P&&(clearInterval(P),P=null),b.innerHTML=`
  ...
  `;let a=d=>d.map((u,p)=>{let f=Number(u.globalRank)||p+1,v=String(u.username||""),$=!!(t.walletAddress&&v.toLowerCase()===t.walletAddress.toLowerCase()),x=$&&t.profileUsername?t.profileUsername:u.displayName||v,A=$?`${t.profileUsername?x:z(v)} (You)`:x.startsWith("0x")&&x.length===42?z(x):x,_=q(A),L=q(tt(u.status)),W=u.nextSeasonDivision?`Division ${u.nextSeasonDivision}`:"Qualify",B=f<=10?"promotion-zone":"safety-zone",ae=f<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
      <div class="leaderboard-row global-row ${$?"user-highlight":""} ${B}" role="listitem">
        <div class="leaderboard-row-left">
          ${ae}
          <span class="leaderboard-rank rank-${f}">${f}</span>
          <span class="leaderboard-username">${_}</span>
        </div>
        <div class="leaderboard-row-score">
          <strong>${Number(u.points)||0} pts</strong>
          <span>${u.prizeEligible?"Prize eligible":"Season rank"} \xB7 ${q(W)}</span>
        </div>
        <div class="leaderboard-row-right">
          <span>${L}</span>
        </div>
      </div>
    `}).join(""),r=d=>{K=d,document.querySelectorAll("[data-leaderboard-view]").forEach(u=>{u.classList.toggle("active",u.dataset.leaderboardView===d)}),document.getElementById("divisionControls")?.toggleAttribute("hidden",d!=="division"),document.getElementById("globalControls")?.toggleAttribute("hidden",d!=="global"),document.getElementById("globalPrizeBox")?.toggleAttribute("hidden",d!=="global")},o=d=>{let u=document.getElementById("leaderboardListContainer");u&&(u.innerHTML=`
      <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
    ...
    `)},s=()=>{r("global"),o(10);let d=document.getElementById("leaderboardListContainer"),u=t.walletAddress?`&walletAddress=${encodeURIComponent(t.walletAddress)}`:"";fetch(E(`/api/leaderboard/global?nocache=1${u}`)).then(p=>p.json()).then(p=>{let f=p.players||[];d&&(d.innerHTML=f.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:a(f));let v=document.getElementById("seasonTimer");P&&clearInterval(P);let $=()=>{let A=new Date(p.seasonEndsAt).getTime()-new Date().getTime();if(A<=0){v&&(v.innerText="Season Finished!"),P&&clearInterval(P);return}let _=Math.floor(A/(1e3*60*60*24)),L=Math.floor(A%(1e3*60*60*24)/(1e3*60*60)),W=Math.floor(A%(1e3*60*60)/(1e3*60)),B=Math.floor(A%(1e3*60)/1e3);v&&(v.innerText=`${_}d ${L}h ${W}m ${B}s`)};$(),P=setInterval($,1e3)}).catch(p=>{console.error("Failed to load global leaderboard:",p),d&&(d.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},i=d=>{r("division");let u=document.getElementById("leaderboardListContainer");u&&d!==void 0&&(u.innerHTML=`
        <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
          ${Array.from({length:6}).map(()=>`
            <div style="height: 52px; background: rgba(255,255,255,0.02); border: 1px solid #1e1f2b; border-radius: 8px; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 16px;">
              <div style="display: flex; align-items: center; gap: 12px; width: 60%;">
      ...
      `);let p=t.walletAddress?`&walletAddress=${encodeURIComponent(t.walletAddress)}`:"",f=d?`&division=${d}`:"";fetch(E(`/api/leaderboard/division?nocache=1${p}${f}`)).then(v=>v.json()).then(v=>{let $=v.divisionNumber||1,x=v.players||[],A=v.totalDivisions||1,_=v.seasonEndsAt;Pe=$;let L=document.getElementById("divisionTitleText");L&&(L.innerText=`Division ${$}`);let W=document.getElementById("divisionSelector");W&&(W.innerHTML=Array.from({length:A},(U,G)=>G+1).map(U=>`
            <option value="${U}" ${U===$?"selected":""}>Division ${U}</option>
          `).join("")),u&&(x.length===0?u.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No active players in this division.</p>`:u.innerHTML=x.map((U,G)=>{let S=G+1,O=t.walletAddress&&U.username.toLowerCase()===t.walletAddress.toLowerCase(),J=O&&t.profileUsername?t.profileUsername:U.displayName||U.username,Ae=q(tt(U.status)),Me=O?`${t.profileUsername?J:z(U.username)} (You)`:J.startsWith("0x")&&J.length===42?z(J):J,Ot=q(Me),Le="safety-zone",Ue='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return S<=2?(Le="promotion-zone",Ue='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):S>=5&&(Le="relegation-zone",Ue='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
                <div class="leaderboard-row ${O?"user-highlight":""} ${Le}" role="listitem" style="display: flex !important; align-items: center !important; justify-content: space-between !important; padding: 12px 16px !important; border-bottom: 1px solid rgba(255, 255, 255, 0.06) !important; margin-bottom: 0 !important; background: transparent !important; font-family: 'Space Grotesk', sans-serif !important;">
                  ...
                  <div style="flex: 1; display: flex; align-items: center; justify-content: center;">
                    <span style="color: #ffffff; font-weight: 750; font-size: 0.95rem; white-space: nowrap;">${U.points} pts</span>
                  </div>
                  <!-- Right Side: Status -->
  ...
  `},qa=async e=>{if(!t.walletAddress){y("Please sign in first.");return}let a=T.find(o=>o.id===e),r=a?j(a):"";if(!a||!r){y("Market is not available.");return}try{M("claim_attempt"),Je();let o=await zt(r,t.walletAddress);M("claim_success"),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ce(t.walletAddress),await H(),y(o.won?`Claimed $${R(o.amountUsdc)}`:"No payout to claim"),C(),fe()}catch(o){M("claim_failed"),y(o instanceof Error?o.message:"Claim failed")}},fe=()=>{if(!b||!k)return;Se?.toggleAttribute("hidden",!0),xe?.toggleAttribute("hidden",!0),X?.toggleAttribute("hidden",!0),he?.classList.remove("active"),ge?.classList.remove("active"),ve?.classList.add("active"),document.body.classList.remove("detail-mode"),k.hidden=!0,b.hidden=!1,b.classList.add("markets-list"),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let e=T.filter(l=>{let g=t.marketPositions[l.id];return g&&g.yesSharesUsdc+g.noSharesUsdc>0}),a=e.filter(l=>(t.marketSnapshots[l.id]?.outcome??0)===0),r=e.filter(l=>(t.marketSnapshots[l.id]?.outcome??0)!==0),o=!!t.walletAddress,s=t.profileUsername||(t.walletAddress?z(t.walletAddress):"Anonymous"),i=q(s),n=q(t.profileUsername||""),m=t.profileNotice?`<div style="margin-top: 14px !important; padding: 10px 12px !important; border-radius: 8px !important; border: 1px solid ${t.profileNotice.type==="error"?"rgba(239, 68, 68, 0.28)":"rgba(16, 185, 129, 0.24)"} !important; background: ${t.profileNotice.type==="error"?"rgba(127, 29, 29, 0.22)":"rgba(6, 95, 70, 0.18)"} !important; color: ${t.profileNotice.type==="error"?"#fca5a5":"#86efac"} !important; font-size: 0.8rem !important; font-weight: 650 !important;">${q(t.profileNotice.message)}</div>`:"",c=s.charAt(0).toUpperCase();b.innerHTML=`

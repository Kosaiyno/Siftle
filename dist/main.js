...
Trade and discuss here: ${l}`},$r=e=>{if(!k||!T)return;let a=qe(e),r=!t.checkedMarketEvidence[e.id],o=G(e),n=t.marketSnapshots[e.id],s=se(e),i=ie(e);s&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let l=Za(e),u=!!(o&&!n),m=n?.yesPriceCents??(o?e.probability:0),f=n?.noPriceCents??(o?100-e.probability:0),p=u?"":o?`${m}\xA2`:"--",h=u?"":o?`${f}\xA2`:"--",d=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},c=!!d.optionId;s&&c&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),s&&!c&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let b=s&&t.marketOrderMode==="sell"&&c?Math.max(0,Number(d.optionSharesUsdc)||0):0,x=b>0?b:Be(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,d),w=b>0?{min:0,max:b}:ea(t.marketOrderMode,t.marketTradeSide,d),$=t.marketOrderMode==="buy"?"$5-$10 USDC":`Up to $${N(w.max)} USDC`,y=!t.walletAddress||t.hasLoadedPortfolioPositions,S=We(e,n),U=Yt(e,n),P=!!U;s||(t.marketTradeSide=dt(t.marketOrderMode,t.marketTradeSide,d));let O=!s&&!S&&!P&&y&&X(t.marketOrderMode,"yes",d),R=!s&&!S&&!P&&y&&X(t.marketOrderMode,"no",d),de=s?!S&&!P&&y&&(t.marketOrderMode==="sell"?c:!c&&!!l):!S&&!P&&y&&X(t.marketOrderMode,t.marketTradeSide,d),_=S?"Market resolved":U||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),ce=S?"Market resolved":U||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),pe=s?x:ta(n,t.marketTradeSide,x,t.marketOrderMode,d),ue=t.marketOrderMode==="buy"?"Buy":"Exit",K=s?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";k.hidden=!0,T.hidden=!1,T.classList.add("fullscreen"),document.body.classList.add("detail-mode"),ir(e),Qt(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&F({force:!t.hasLoadedPortfolioPositions});let De=s?!!d.optionId:d.yesSharesUsdc>0||d.noSharesUsdc>0,ye="";if(s&&De&&t.walletAddress){let M=n?.optionPools?.[d.optionId||""]||0,I=n?.volumeUsdc||0,V=d.optionSharesUsdc&&M>0?d.optionSharesUsdc/M*I:0;ye=`
      ...
      <div class="user-market-position-box" style="margin: 16px 0; padding: 16px; background: rgba(59, 130, 246, 0.05); border: 1px solid rgba(59, 130, 246, 0.15); border-radius: 12px; font-family: 'Space Grotesk', sans-serif;">
        ...
        ${aa(d,n).map(I=>`
          <div style="display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 12px; margin-bottom: 8px;">
            <div>
              <span style="font-size: 0.72rem; color: var(--market-text-muted); display: block; margin-bottom: 2px;">${I.label}</span>
              <strong style="font-size: 0.95rem; color: var(--market-text-main);">${N(I.shares)}</strong>
            </div>
            <div>
              <span style="font-size: 0.72rem; color: var(--market-text-muted); display: block; margin-bottom: 2px;">Projected payout</span>
              <strong style="font-size: 0.95rem; color: var(--market-text-main);">$${N(I.payout)}</strong>
            </div>
          </div>
        `).join("")}
        ...
        <div class="market-detail-main">
          ...
          <section class="market-evidence-thread">
            ...
            <div class="market-thread-timeline">
              ${r?ur(3):a.evidence.length===0?'<div class="portfolio-empty compact">Related news is still loading for this market.</div>':a.evidence.map(M=>{let I=qt(e,M),V=t.unlockingSummaryUrl===M.sourceUrl;return`
                <article class="market-thread-update">
                  ...
                  <div class="market-thread-update-content">
                    ...
                    ${Ce(I)}
                    ${V?`<div style="margin-top: 12px;">${he()}</div>`:oe(I)?t.loadingSummaryUrl===M.sourceUrl?`<div style="margin-top: 12px;">${he()}</div>`:nt(I)?`<div style="margin-top: 12px;">${it(I)}</div>`:`<div style="margin-top: 12px;">${rt(re(I,t.aiSummaries[M.sourceUrl]),I)}</div>`:""}
                  </div>
                </article>
              `}).join("")}
            </div>
      ...
      <div class="trade-drawer ${t.tradeDrawerOpen?"open":""}" id="tradeDrawer">
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${s?i.map(M=>{let I=n?.optionPools?.[M.id]||0,V=t.marketTradeOptionId===M.id||d.optionId===M.id,ut=S||P||t.marketOrderMode==="sell"||c||!y;return`
  ...
  `;let a=(d="2026-07-19T23:59:59.000Z")=>{let c=document.getElementById("seasonTimer");J&&clearInterval(J);let b=()=>{let w=new Date(d).getTime()-new Date().getTime();if(w<=0){c&&(c.innerText="Season Finished!"),J&&clearInterval(J);return}let $=Math.floor(w/(1e3*60*60*24)),y=Math.floor(w%(1e3*60*60*24)/(1e3*60*60)),S=Math.floor(w%(1e3*60*60)/(1e3*60)),U=Math.floor(w%(1e3*60)/1e3);c&&(c.innerText=`${$}d ${y}h ${S}m ${U}s`)};b(),J=setInterval(b,1e3)};a();let r=d=>d.map((c,b)=>{let x=Number(c.globalRank)||b+1,w=String(c.username||""),$=!!(t.walletAddress&&w.toLowerCase()===t.walletAddress.toLowerCase()),y=$&&t.profileUsername?t.profileUsername:c.displayName||w,S=$?`${t.profileUsername?y:z(w)} (You)`:y.startsWith("0x")&&y.length===42?z(y):y,U=L(S),P=L(Ve(c.status)),O=c.nextSeasonDivision?`Division ${c.nextSeasonDivision}`:"Qualify",R=x<=10?"promotion-zone":"safety-zone",de=x<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
    ...
    `}).join(""),n=d=>{Z=d,document.querySelectorAll("[data-leaderboard-view]").forEach(c=>{c.classList.toggle("active",c.dataset.leaderboardView===d)}),document.getElementById("divisionControls")?.toggleAttribute("hidden",d!=="division"),document.getElementById("globalControls")?.toggleAttribute("hidden",d!=="global"),document.getElementById("globalPrizeBox")?.toggleAttribute("hidden",d!=="global")},s=d=>{let c=document.getElementById("leaderboardListContainer");c&&(c.innerHTML=`
      <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
    ...
    `)},i=()=>{n("global");let d=document.getElementById("leaderboardListContainer"),c="global",b=kt(c);b&&d?(d.innerHTML=St()+r(b.players),b.seasonEndsAt&&a(b.seasonEndsAt)):s(10);let x=new URLSearchParams;t.walletAddress&&x.set("walletAddress",t.walletAddress);let w=x.toString();fetch(E(`/api/leaderboard/global${w?`?${w}`:""}`)).then($=>$.json()).then($=>{let y=wt($.players||[],b?.players||[],!0);d&&(d.innerHTML=y.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:r(y)),Ze(c,{players:y,seasonEndsAt:$.seasonEndsAt}),a($.seasonEndsAt)}).catch($=>{console.error("Failed to load global leaderboard:",$),d&&(d.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},l=d=>{n("division");let c=document.getElementById("leaderboardListContainer"),b=`division_${d||ke||"current"}`,x=kt(b);x&&c?(c.innerHTML=St()+o(x.players),x.divisionNumber&&(ke=x.divisionNumber),x.seasonEndsAt&&a(x.seasonEndsAt)):c&&d!==void 0&&(c.innerHTML=`
        <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
          ${Array.from({length:6}).map(()=>`
            <div style="height: 52px; background: rgba(255,255,255,0.02); border: 1px solid #1e1f2b; border-radius: 8px; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 16px;">
              <div style="display: flex; align-items: center; gap: 12px; width: 60%;">
      ...
      `);let w=new URLSearchParams;t.walletAddress&&w.set("walletAddress",t.walletAddress),d&&w.set("division",String(d));let $=w.toString();fetch(E(`/api/leaderboard/division${$?`?${$}`:""}`)).then(y=>y.json()).then(y=>{let S=y.divisionNumber||1,U=wt(y.players||[],x?.players||[],!1),P=y.totalDivisions||1,O=y.seasonEndsAt;ke=S;let R=document.getElementById("divisionTitleText");R&&(R.innerText=`Division ${S}`);let de=document.getElementById("divisionSelector");de&&(de.innerHTML=Array.from({length:P},(_,ce)=>ce+1).map(_=>`
            <option value="${_}" ${_===S?"selected":""}>Division ${_}</option>
          `).join("")),c&&(U.length===0?c.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No active players in this division.</p>`:c.innerHTML=U.map((_,ce)=>{let pe=ce+1,ue=t.walletAddress&&_.username.toLowerCase()===t.walletAddress.toLowerCase(),K=ue&&t.profileUsername?t.profileUsername:_.displayName||_.username,pt=L(Ve(_.status)),De=ue?`${t.profileUsername?K:z(_.username)} (You)`:K.startsWith("0x")&&K.length===42?z(K):K,ye=L(De),M="safety-zone",I='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return pe<=2?(M="promotion-zone",I='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):pe>=5&&(M="relegation-zone",I='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`

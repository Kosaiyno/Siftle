...
`,pe=()=>{if(!b)return;let e=oa();if(b.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){b.innerHTML=Ua(4);return}if(e.length===0){let a=t.showSaved?[]:t.stories;if(a.length>0){b.innerHTML=a.map(r=>`
  ...
  <article class="thread-item">
    ...
    <div class="thread-item-body">
      ...
      ${te(e)?t.loadingSummaryUrl===e.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${We(G(e,t.aiSummaries[e.sourceUrl]||e.ai_summary))}</div>`:""}
    </div>
  </article>
`,Ha=()=>{if(!k||!b)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(b.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){k.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){k.innerHTML=`
  ...
  `},Ra=e=>{let a=t.marketSnapshots[e.id],r=W(e),o=a?.yesPriceCents,s=o??e.probability,i=`${s}%`,n=o===void 0?r?"Loading Arc pools":"Arc setup required":`Yes ${o}\xA2 \xB7 No ${100-o}\xA2`,m=o===void 0?`Yes ${e.probability}c - No ${100-e.probability}c`:n,c=Te(e);return`
  ...
  `},ja=e=>{let a=Te(e),o=t.marketSnapshots[e.id]?.yesPriceCents??e.probability,s=100-o,i=a.evidence[0],n=i?i.headline:"No updates yet",m=`${window.location.origin}${window.location.pathname}#market-${e.id}`;return`\u{1F6A8} *Siftle Market Alert* \u{1F6A8}
...
Trade and discuss here: ${m}`},Wa=e=>{if(!b||!k)return;let a=Te(e),r=!t.checkedMarketEvidence[e.id],o=W(e),s=t.marketSnapshots[e.id],i=!!(o&&!s),n=s?.yesPriceCents??(o?e.probability:0),m=s?.noPriceCents??(o?100-e.probability:0),c=i?"":o?`${n}\xA2`:"--",l=i?"":o?`${m}\xA2`:"--",g=ze(Number(t.marketTradeAmount)||0),d=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},u=!t.walletAddress||t.hasLoadedPortfolioPositions,p=Ye(e,s),f=xt(e,s),v=!!f;t.marketTradeSide=Ge(t.marketOrderMode,t.marketTradeSide,d);let $=!p&&!v&&u&&Q(t.marketOrderMode,"yes",d),x=!p&&!v&&u&&Q(t.marketOrderMode,"no",d),E=!p&&!v&&u&&Q(t.marketOrderMode,t.marketTradeSide,d),P=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),M=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),q=Pt(s,t.marketTradeSide,g,t.marketOrderMode,d),D=t.marketOrderMode==="buy"?"Buy":"Exit",F=o?"Arc testnet live":"Contract not deployed";b.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),Aa(e),Ut(e),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let L=d.yesSharesUsdc>0||d.noSharesUsdc>0,J="";L&&t.walletAddress&&(J=`
      ...
      <article class="market-detail-card">
        <div class="market-detail-main">
          <div class="market-detail-topline">
            <span class="category-chip ${e.category}">${N(e.category)}</span>
            <span class="market-status-pill">${F}</span>
          </div>
          <h2>${e.question}</h2>
          ${J}
          ${a.imageUrl?`
              ...
              ${r?Pa(3):a.evidence.length===0?'<div class="portfolio-empty compact">Market thread is still being prepared for this match.</div>':a.evidence.map(S=>`
                <article class="market-thread-update">
                  ...
                  <div class="market-thread-update-content">
                    ...
                    ${te(we(e,S))?t.loadingSummaryUrl===S.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${We(G(we(e,S),t.aiSummaries[S.sourceUrl]))}</div>`:""}
                  </div>
                </article>
              `).join("")}
            </div>
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${i?`
              ...
              `:`
                <button type="button" class="market-side yes ${t.marketTradeSide==="yes"?"active":""} ${$?"":"disabled"}" data-market-trade-side="yes" ${$?"":"disabled"} title="${$?"Yes":P}">
                  <span>Yes</span>
                  <strong>${c}</strong>
                  ${$?"":`<small>${P}</small>`}
                </button>
  ...
  `;let a=d=>d.map((u,p)=>{let f=Number(u.globalRank)||p+1,v=String(u.username||""),$=!!(t.walletAddress&&v.toLowerCase()===t.walletAddress.toLowerCase()),x=$&&t.profileUsername?t.profileUsername:u.displayName||v,E=$?`${t.profileUsername?x:Y(v)} (You)`:x.startsWith("0x")&&x.length===42?Y(x):x,P=z(E),M=z(tt(u.status)),q=u.nextSeasonDivision?`Division ${u.nextSeasonDivision}`:"Qualify",D=f<=10?"promotion-zone":"safety-zone",F=f<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
      <div class="leaderboard-row global-row ${$?"user-highlight":""} ${D}" role="listitem">
        ...
        <div class="leaderboard-row-score">
          ...
          <span>${u.prizeEligible?"Prize eligible":"Season rank"} \xB7 ${z(q)}</span>
        </div>
    ...
    `)},s=()=>{r("global"),o(10);let d=document.getElementById("leaderboardListContainer"),u=new URLSearchParams;t.walletAddress&&u.set("walletAddress",t.walletAddress);let p=u.toString();fetch(C(`/api/leaderboard/global${p?`?${p}`:""}`)).then(f=>f.json()).then(f=>{let v=f.players||[];d&&(d.innerHTML=v.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:a(v));let $=document.getElementById("seasonTimer");I&&clearInterval(I);let x=()=>{let P=new Date(f.seasonEndsAt).getTime()-new Date().getTime();if(P<=0){$&&($.innerText="Season Finished!"),I&&clearInterval(I);return}let M=Math.floor(P/(1e3*60*60*24)),q=Math.floor(P%(1e3*60*60*24)/(1e3*60*60)),D=Math.floor(P%(1e3*60*60)/(1e3*60)),F=Math.floor(P%(1e3*60)/1e3);$&&($.innerText=`${M}d ${q}h ${D}m ${F}s`)};x(),I=setInterval(x,1e3)}).catch(f=>{console.error("Failed to load global leaderboard:",f),d&&(d.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},i=d=>{r("division");let u=document.getElementById("leaderboardListContainer");u&&d!==void 0&&(u.innerHTML=`
        <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
          ${Array.from({length:6}).map(()=>`
            <div style="height: 52px; background: rgba(255,255,255,0.02); border: 1px solid #1e1f2b; border-radius: 8px; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 16px;">
              <div style="display: flex; align-items: center; gap: 12px; width: 60%;">
      ...
      `);let p=new URLSearchParams;t.walletAddress&&p.set("walletAddress",t.walletAddress),d&&p.set("division",String(d));let f=p.toString();fetch(C(`/api/leaderboard/division${f?`?${f}`:""}`)).then(v=>v.json()).then(v=>{let $=v.divisionNumber||1,x=v.players||[],E=v.totalDivisions||1,P=v.seasonEndsAt;Ee=$;let M=document.getElementById("divisionTitleText");M&&(M.innerText=`Division ${$}`);let q=document.getElementById("divisionSelector");q&&(q.innerHTML=Array.from({length:E},(L,J)=>J+1).map(L=>`
  ...
  `},Fa=async e=>{if(!t.walletAddress){y("Please sign in first.");return}let a=T.find(o=>o.id===e),r=a?W(a):"";if(!a||!r){y("Market is not available.");return}try{A("claim_attempt"),Je();let o=await zt(r,t.walletAddress);A("claim_success"),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ce(t.walletAddress),await H(),y(o.won?`Claimed $${R(o.amountUsdc)}`:"No payout to claim"),U(),fe()}catch(o){A("claim_failed"),y(o instanceof Error?o.message:"Claim failed")}},fe=()=>{if(!b||!k)return;Se?.toggleAttribute("hidden",!0),xe?.toggleAttribute("hidden",!0),ee?.toggleAttribute("hidden",!0),he?.classList.remove("active"),ge?.classList.remove("active"),ve?.classList.add("active"),document.body.classList.remove("detail-mode"),k.hidden=!0,b.hidden=!1,b.classList.add("markets-list"),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let e=T.filter(l=>{let g=t.marketPositions[l.id];return g&&g.yesSharesUsdc+g.noSharesUsdc>0}),a=e.filter(l=>(t.marketSnapshots[l.id]?.outcome??0)===0),r=e.filter(l=>(t.marketSnapshots[l.id]?.outcome??0)!==0),o=!!t.walletAddress,s=t.profileUsername||(t.walletAddress?Y(t.walletAddress):"Anonymous"),i=z(s),n=z(t.profileUsername||""),m=t.profileNotice?`<div style="margin-top: 14px !important; padding: 10px 12px !important; border-radius: 8px !important; border: 1px solid ${t.profileNotice.type==="error"?"rgba(239, 68, 68, 0.28)":"rgba(16, 185, 129, 0.24)"} !important; background: ${t.profileNotice.type==="error"?"rgba(127, 29, 29, 0.22)":"rgba(6, 95, 70, 0.18)"} !important; color: ${t.profileNotice.type==="error"?"#fca5a5":"#86efac"} !important; font-size: 0.8rem !important; font-weight: 650 !important;">${z(t.profileNotice.message)}</div>`:"",c=s.charAt(0).toUpperCase();b.innerHTML=`
    <section class="portfolio-surface">
      ...
      ${t.loadingPortfolioPositions?Ea(2):t.walletAddress?e.length===0?'<div class="portfolio-empty">No positions found for this wallet yet. Confirmed trades will appear here after the Arc transaction settles.</div>':`

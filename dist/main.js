...
`,St=e=>{let a=e.type==="tweet",r='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: text-top; color: var(--color-text-primary);"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',o='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 12px; height: 12px; display: inline-block; vertical-align: text-top; margin-right: 4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';return`
  ...
  `},be=()=>{if(!w)return;let e=Ea();if(w.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){w.innerHTML=hr(4);return}let a=U(t.newsSearchQuery.trim()),o=`
    <section class="news-feed-search-shell">
      ${a?`<div class="news-feed-search-copy"><p>${e.length} matches for "${a}".</p></div>`:""}
...
Trade and discuss here: ${l}`},Pr=e=>{if(!w||!S)return;let a=je(e),r=!t.checkedMarketEvidence[e.id],o=K(e),s=t.marketSnapshots[e.id],n=te(e),i=re(e);n&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let l=er(e),c=!!(o&&!s),u=s?.yesPriceCents??(o?e.probability:0),p=s?.noPriceCents??(o?100-e.probability:0),d=c?"":o?`${u}\xA2`:"--",b=c?"":o?`${p}\xA2`:"--",m=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},g=!!m.optionId;n&&g&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),n&&!g&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let T=n&&t.marketOrderMode==="sell"&&g?Math.max(0,Number(m.optionSharesUsdc)||0):0,B=T>0?T:Be(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,m),v=T>0?{min:0,max:T}:Qt(t.marketOrderMode,t.marketTradeSide,m),f=t.marketOrderMode==="buy"?"exactly $2.00 USDC":`Up to $${H(v.max)} USDC`,A=!t.walletAddress||t.hasLoadedPortfolioPositions,k=Fe(e,s),$=Ft(e,s),x=!!$;n||(t.marketTradeSide=ct(t.marketOrderMode,t.marketTradeSide,m));let M=!n&&!k&&!x&&A&&se(t.marketOrderMode,"yes",m),O=!n&&!k&&!x&&A&&se(t.marketOrderMode,"no",m),Y=n?!k&&!x&&A&&(t.marketOrderMode==="sell"?g:!g&&!!l):!k&&!x&&A&&se(t.marketOrderMode,t.marketTradeSide,m),q=k?"Market resolved":$||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),V=k?"Market resolved":$||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),Z=n?B:Xt(s,t.marketTradeSide,B,t.marketOrderMode,m),Q=t.marketOrderMode==="buy"?"Buy":"Exit",ue=n?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";w.hidden=!0,S.hidden=!1,S.classList.add("fullscreen"),document.body.classList.add("detail-mode"),dr(e),Vt(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&z({force:!t.hasLoadedPortfolioPositions});let ut=n?!!m.optionId:m.yesSharesUsdc>0||m.noSharesUsdc>0,qe="";n&&ut&&t.walletAddress?qe=`
  ...
  `,((v="2026-07-19T23:59:59.000Z")=>{let f=document.getElementById("seasonTimer");ee&&clearInterval(ee);let A=()=>{let $=new Date(v).getTime()-new Date().getTime();if($<=0){f&&(f.innerText="Season Finished!"),ee&&clearInterval(ee);return}let x=Math.floor($/(1e3*60*60*24)),M=Math.floor($%(1e3*60*60*24)/(1e3*60*60)),O=Math.floor($%(1e3*60*60)/(1e3*60)),Y=Math.floor($%(1e3*60)/1e3);f&&(f.innerText=`${x}d ${M}h ${O}m ${Y}s`)};A(),ee=setInterval(A,1e3)})();let r=v=>v.map((f,A)=>{let k=Number(f.globalRank)||A+1,$=String(f.username||""),x=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),M=x&&t.profileUsername?t.profileUsername:f.displayName||$,O=x?`${t.profileUsername?M:R($)} (You)`:M.startsWith("0x")&&M.length===42?R(M):M,Y=U(O),q=kt(f.status);if(f.totalTrades!==void 0&&f.aiBriefingUnlocks!==void 0){let We=wt(f.status);q=`${We.wins}W - ${We.losses}L \xB7 ${f.totalTrades} trades \xB7 ${f.aiBriefingUnlocks} unlocks`}let V=U(q),Z=f.nextSeasonDivision?`Division ${f.nextSeasonDivision}`:"Qualify",Q=k<=10?"promotion-zone":"safety-zone",ue=k<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
    ...
    `}let o=Cr(r?.outcome),s=ea(a,r),n=s.reduce((g,T)=>Math.max(g,T.payout),0),i=a.yesSharesUsdc+a.noSharesUsdc,l=r?.outcome??0,c=He().has(e.id),u=l===1?a.yesSharesUsdc:l===2?a.noSharesUsdc:0,p=l===1?r?.yesSharesUsdc??0:l===2?r?.noSharesUsdc??0:0,d=r?.volumeUsdc??0,b=u>0&&p>0?u/p*d:0,m=l===0?"":c?'<span style="color: #34d399; font-size: 0.82rem; font-weight: 800;">Claimed</span>':b>0?`<button type="button" class="connect-wallet-btn" data-claim-market="${e.id}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 6px !important; padding: 8px 14px !important; font-size: 0.82rem !important; font-weight: 700 !important; cursor: pointer !important;">Claim $${H(b)}</button>`:'<span style="color: #ef4444; font-size: 0.82rem; font-weight: 800;">Lost</span>';return`
    <article class="portfolio-position-card">
      <div class="portfolio-position-top">
        <span class="category-chip ${e.category}">${W(e.category)}</span>
        <span>${o}</span>
  ...
  `},Ir=async e=>{if(!t.walletAddress){y("Please sign in first.");return}let a=Me().find(o=>o.id===e),r=a?K(a):"";if(!a||!r){y("Market is not available.");return}try{t.claimingMarketIds[a.id]=!0,F(),P("claim_attempt"),pt();let o=await fa(r,t.walletAddress);P("claim_success"),o.won&&or(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ne(t.walletAddress),await z(),y(o.won?`Claimed $${H(o.amountUsdc)}`:"No payout to claim"),D(),F()}catch(o){P("claim_failed"),y(o instanceof Error?o.message:"Claim failed")}finally{delete t.claimingMarketIds[a.id],F()}},Er=e=>{if(!e)return"";let a=t.referralData,r=a?.referrals?.length?a.referrals.map(s=>{let n=s.displayName||R(s.walletAddress),i=s.remaining<=0;return`
  ...
  `},h=()=>{if(Ot.forEach(e=>{let a=e.dataset.bottomNav;e.classList.toggle("active",a==="saved"?t.showSaved:a===t.activeSurface&&!t.showSaved)}),t.activeSurface==="markets"){sa();return}if(t.activeSurface==="portfolio"){F();return}if(t.activeSurface==="leaderboard"){na();return}ia(),Gt(),be(),Ar(),j&&(j.value=t.activeArchiveDate??"")};Qe.textContent=Ht();le?.addEventListener("click",e=>{let r=e.target.closest("[data-category]");r&&(t.activeCategory=r.dataset.category,window.history.pushState({},"","#feed"),J(),h(),Ee(),ae(t.activeCategory))});w?.addEventListener("input",e=>{let a=e.target;if(a.id!=="newsSearchInput")return;let r=a.selectionStart??a.value.length,o=a.selectionEnd??a.value.length;t.newsSearchQuery=a.value,be();let s=w?.querySelector("#newsSearchInput");s&&(s.focus(),s.setSelectionRange(r,o))});Ae?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="markets",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,window.history.pushState({},"","#markets"),J(),h()});Le?.addEventListener("click",()=>{t.activeSurface="feed",t.showSaved=!1,window.history.pushState({},"","#feed"),J(),h(),Ee(),ae(t.activeCategory)});Ue?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="portfolio",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=!1,window.history.pushState({},"","#portfolio"),J(),h()});oe?.addEventListener("click",()=>{t.walletAddress?(window.location.hash="#portfolio",Ie()):Pe()});document.addEventListener("click",e=>{let a=e.target,r=a.closest(".copy-address-btn");if(r){let i=r.getAttribute("data-address");i&&navigator.clipboard.writeText(i).then(()=>{y("Wallet address copied!")})}let o=a.closest("[data-claim-market]");if(o){let i=o.getAttribute("data-claim-market");i&&Ir(i);return}if(a.closest("[data-open-referrals]")){t.referralPanelOpen=!t.referralPanelOpen,!t.referralData&&!t.loadingReferralData&&$e(),F();return}if(a.closest("[data-close-referrals]")){t.referralPanelOpen=!1,F();return}if(a.closest("[data-refresh-referrals]")){t.referralError=null,$e(),F();return}let s=a.closest("[data-copy-referral-code]");if(s){let i=s.getAttribute("data-copy-referral-code")||"";i&&navigator.clipboard.writeText(i).then(()=>y("Invite code copied"));return}let n=a.closest("[data-copy-referral-link]");if(n){let i=n.getAttribute("data-copy-referral-link")||"";i&&navigator.clipboard.writeText(i).then(()=>y("Invite link copied"));return}a.closest("[data-connect-wallet]")&&(t.walletAddress?Ct():Pe())});Ot.forEach(e=>{e.addEventListener("click",()=>{let a=e.dataset.bottomNav;t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=a==="saved",a==="markets"?(t.activeSurface="markets",window.history.pushState({},"","#markets")):a==="portfolio"?(t.activeSurface="portfolio",window.history.pushState({},"","#portfolio")):a==="leaderboard"?(t.activeSurface="leaderboard",window.history.pushState({},"","#leaderboard")):(t.activeSurface="feed",window.history.pushState({},"","#feed"),Ee(),ae(t.activeCategory),a==="saved"&&(ar(),rt(),xe())),J(),h()})});j?.addEventListener("change",()=>{t.activeArchiveDate=j.value||null,window.history.pushState({},"","#feed"),J(),h(),ae(t.activeCategory)});Aa?.addEventListener("click",()=>{t.activeArchiveDate=null,j&&(j.value=""),window.history.pushState({},"","#feed"),J(),h(),ae(t.activeCategory)});w?.addEventListener("click",async e=>{let a=e.target;if(a.closest("#editUsernameBtn")){let m=w?.querySelector(".username-display-row"),g=w?.querySelector("#usernameEditForm");if(m&&g){m.style.display="none",g.style.display="flex";let T=g.querySelector("#usernameInput");T&&T.focus()}return}if(a.closest("#cancelUsernameBtn")){let m=w?.querySelector(".username-display-row"),g=w?.querySelector("#usernameEditForm");m&&g&&(m.style.display="flex",g.style.display="none");return}let s=a.closest("#saveUsernameBtn");if(s){let g=w?.querySelector("#usernameEditForm")?.querySelector("#usernameInput");if(g){let T=g.value.trim().slice(0,15),B=s,v=B.textContent||"Save";B.disabled=!0,B.textContent="Saving...",tr(T),t.profileNotice=null;try{t.walletAddress&&await pe(!1),t.profileNotice={type:"success",message:"Username saved to your shared leaderboard profile."},y("Username updated"),F()}catch(f){let A=f instanceof Error?f.message:"Username save failed";t.profileNotice={type:"error",message:A},y(A),B.disabled=!1,B.textContent=v,F()}}return}let n=a.closest("[data-timeframe]");if(n){let m=n.dataset.timeframe;t.activeMarketTimeframe=m,sa();return}let i=a.closest("[data-market-id]");if(i){t.selectedMarketId=i.dataset.marketId??null,P("market_view"),window.history.pushState({},"",`#market-${t.selectedMarketId}`),h(),window.scrollTo({top:0,behavior:"smooth"});return}if(a.closest(".read-tweet-btn")){e.stopPropagation();let m=a.closest("[data-story-id]");m&&et(Number(m.dataset.storyId),!0);return}let c=a.closest("[data-thread-story-id]"),u=a.closest("[data-export-id]"),p=a.closest("[data-export-action]"),d=a.closest("[data-story-id]");if(c){e.stopPropagation();let m=t.stories.find(g=>g.id===Number(c.dataset.threadStoryId));m&&Fa(m);return}let b=a.closest(".mobile-bookmark-btn, .bookmark-button");if(b){e.stopPropagation();let m=b.dataset.bookmarkUrl||"",g=t.stories.find(T=>T.sourceUrl===m);if(!g)return;g.saved=!g.saved,g.saved?ie.add(m):ie.delete(m),Ma(),y(g.saved?"Saved to your list":"Removed from saved"),be();return}if(p){e.stopPropagation(),$r(Number(p.dataset.exportStoryId),p.dataset.exportAction);return}if(u){e.stopPropagation();let m=Number(u.dataset.exportId);t.activeShareStoryId=t.activeShareStoryId===m?null:m,be();return}d&&(a.closest("a")||et(Number(d.dataset.storyId),!0))});w?.addEventListener("keydown",e=>{let r=e.target.closest("[data-story-id]");!r||e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),et(Number(r.dataset.storyId)))});S?.addEventListener("click",e=>{let a=e.target,r=a.closest("[data-unlock-briefing]");if(r){let p=t.stories.find(d=>d.id===Number(r.dataset.unlockBriefing));p&&Xe(p);return}let o=a.closest("[data-unlock-briefing-url]");if(o){let p=decodeURIComponent(o.dataset.unlockBriefingUrl||""),d=za(p);d&&(ce(d)?Te(d):Xe(d));return}if(a.closest("[data-back-markets]")){t.selectedMarketId=null,t.tradeDrawerOpen=!1,window.history.pushState({},"","#markets"),h();return}if(a.closest("#openTradeDrawerBtn")){let p=I.find(m=>m.id===t.selectedMarketId);if(p){if(Fe(p,t.marketSnapshots[p.id])){y("This market is resolved and can no longer be traded.");return}if(Ft(p,t.marketSnapshots[p.id])){y("Trading is locked 20 minutes before kickoff.");return}}t.tradeDrawerOpen=!0,P("trade_drawer_open");let d=S.querySelector("#tradeDrawer"),b=S.querySelector("#tradeDrawerBackdrop");d?.classList.add("open"),b?.classList.add("open");return}if(a.closest("#closeTradeDrawerBtn")||a.closest("#tradeDrawerBackdrop")){t.tradeDrawerOpen=!1;let p=S.querySelector("#tradeDrawer"),d=S.querySelector("#tradeDrawerBackdrop");p?.classList.remove("open"),d?.classList.remove("open");return}if(a.closest("#shareWhatsAppBtn")){let p=I.find(d=>d.id===t.selectedMarketId);if(p){let d=Ur(p),b=`https://api.whatsapp.com/send?text=${encodeURIComponent(d)}`;window.open(b,"_blank")}return}let s=a.closest("[data-market-trade]");if(s&&t.selectedMarketId){let p=s.dataset.marketTrade;cr(t.selectedMarketId,p);return}let n=a.closest("[data-market-option-trade]");if(n&&t.selectedMarketId){let p=n.dataset.marketOptionTrade||t.marketTradeOptionId||"";Tr(t.selectedMarketId,p);return}let i=a.closest("[data-market-option-id]");if(i){if(i.disabled||i.classList.contains("disabled"))return;t.marketTradeOptionId=i.dataset.marketOptionId||null,h();return}let l=a.closest("[data-market-trade-side]");if(l){if(l.disabled||l.classList.contains("disabled"))return;let p=I.find(m=>m.id===t.selectedMarketId),d=p?t.marketPositions[p.id]:void 0,b=l.dataset.marketTradeSide;if(!se(t.marketOrderMode,b,d))return;t.marketTradeSide=b,h();return}let c=a.closest("[data-market-order-mode]");if(c){t.marketOrderMode=c.dataset.marketOrderMode;let p=I.find(b=>b.id===t.selectedMarketId),d=p?t.marketPositions[p.id]:void 0;t.marketTradeSide=ct(t.marketOrderMode,t.marketTradeSide,d),t.marketTradeAmount=Be(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,d),h();return}a.closest("[data-back-to-feed]")&&Wa()});S?.addEventListener("input",e=>{let a=e.target;if(!a.matches("[data-market-amount]"))return;let r=I.find(c=>c.id===t.selectedMarketId),o=r?t.marketSnapshots[r.id]:void 0,s=r?t.marketPositions[r.id]:void 0,n=Number(a.value);t.marketTradeAmount=Number.isFinite(n)?n:0;let i=r&&te(r)?t.marketTradeAmount:Xt(o,t.marketTradeSide,t.marketTradeAmount,t.marketOrderMode,s),l=S.querySelector(".market-inline-payout strong");l&&(l.textContent=`$${H(i)}`)});S?.addEventListener("focusin",e=>{e.target.matches("[data-market-amount]")&&document.body.classList.add("market-amount-focused")});S?.addEventListener("focusout",e=>{let a=e.target;if(a.matches("[data-market-amount]")){let r=I.find(s=>s.id===t.selectedMarketId),o=r?t.marketPositions[r.id]:void 0;t.marketTradeAmount=Be(Number(a.value)||0,t.marketOrderMode,t.marketTradeSide,o),a.value=String(t.marketTradeAmount),window.setTimeout(()=>document.body.classList.remove("market-amount-focused"),120)}});window.addEventListener("popstate",Ie);window.addEventListener("hashchange",Ie);window.addEventListener("focus",async()=>{if(t.walletAddress){let e=t.walletBalance,a=await ne(t.walletAddress);t.walletBalance=a,D(),(!e||parseFloat(e)===0)&&parseFloat(a)>0&&(console.log("[X402] Balance changed from 0 to positive. Triggering Gateway warmup..."),va())}});Ge?.addEventListener("click",()=>{if(!Je||!Ge)return;let e=!Je.hidden;Je.hidden=e,Ge.setAttribute("aria-expanded",String(!e))});document.addEventListener("click",e=>{let a=e.target;!a.closest(".share-control")&&t.activeShareStoryId!==null&&(t.activeShareStoryId=null,be());let r=a.closest("[data-menu-action]");if(!r)return;let o={today:"Showing today's brief",saved:`${t.stories.filter(s=>s.saved).length} saved stories`,archive:t.archiveDates.length>0?"Choose a saved day from the archive selector":"No saved days yet"};L&&(L.textContent=o[r.dataset.menuAction??"today"]),r.dataset.menuAction==="today"&&(t.showSaved=!1,t.activeArchiveDate=null,j&&(j.value=""),J(),Ee(),ae(t.activeCategory)),r.dataset.menuAction==="saved"&&(ia(),rt(),xe(),t.showSaved=!0,document.querySelector("#archiveControls")?.classList.remove("mobile-open"),J(),h())});var Br=async()=>{try{let e=await fetch(C("/api/summary/unlock-config"));e.ok&&(t.unlockConfig=await e.json(),h())}catch(e){console.error("Failed to prefetch unlock config:",e)}},Dr=()=>{window.setInterval(async()=>{try{let e=new AbortController,a=window.setTimeout(()=>e.abort(),3500),r=await fetch(C("/api/markets"),{signal:e.signal});if(window.clearTimeout(a),r.ok){let o=await r.json();Array.isArray(o)&&o.length>0&&(I=o,t.activeSurface==="markets"&&h())}}catch(e){console.warn("Background market poll failed:",e)}},3e4)};h();D();Br();ae(t.activeCategory);Dr();Ta().then(()=>{lr(),h(),D(),window.setTimeout(_r,1200),Ga()});var Nr=document.querySelector("#mobileArchiveCard"),he=document.querySelector("#archiveControls");Nr?.addEventListener("click",()=>{if(!he)return;he.classList.toggle("mobile-open")&&setTimeout(()=>he.scrollIntoView({behavior:"smooth",block:"center"}),50)});var Or=document.querySelector("#archivePill");Or?.addEventListener("click",e=>{if(e.stopPropagation(),!he)return;he.classList.toggle("mobile-open")&&setTimeout(()=>he.scrollIntoView({behavior:"smooth",block:"center"}),50)});var Ne=!1,Ut=!1,_r=()=>{Ut||(Ut=!0,(async()=>{let e=await Oe();if(Ne=!!e,e){t.walletConnecting=!0,D();try{let a=await ga();Ne=!1,t.walletConnecting=!1,a?(t.walletAddress=await Oe(),t.walletAddress&&(ge(),t.walletBalance=await ne(t.walletAddress),await z()),D(),t.activeSurface==="portfolio"&&h()):(t.walletAddress=null,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ge(),y("Session expired. Please sign in again."),D(),h())}catch(a){console.warn(a),Ne=!1,t.walletConnecting=!1,t.walletAddress=null,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ge(),y("Session expired. Please sign in again."),D(),h()}}await ha(a=>{Ne||(t.walletAddress=a,t.walletBalance=null,t.referralData=null,t.referralError=null,t.referralPanelOpen=!1,ge(),a&&pe(!1).catch(r=>console.error("Failed to report leaderboard entry:",r)),t.marketPositions={},t.hasLoadedPortfolioPositions=!1,D(),a?($e(),ne(a).then(r=>{t.walletBalance=r,D(),t.activeSurface==="portfolio"&&h()}),z()):t.activeSurface==="portfolio"&&h())})})())};P("app_open");document.addEventListener("click",e=>{let r=e.target.closest("a, button");if(r){let o=r.className||"",s=typeof o=="string"?o:r.getAttribute("class")||"",n=r.getAttribute("href")||"";!(r.hasAttribute("data-unlock-briefing")||r.hasAttribute("data-unlock-briefing-url")||r.classList.contains("summary-btn")||r.textContent?.trim()==="AI briefing"||r.textContent?.trim().includes("Unlock via"))&&(s.includes("source-button")||s.includes("source-btn")||s.includes("source-link")||r.textContent?.trim()==="Open source")&&!s.includes("disabled")&&n!=="#"&&P("open_source")}},!0);

...
`},gr=async(e,a)=>{if(!t.walletAddress){v("Session expired or wallet not connected. Please sign in."),je();return}let r=Me().find(m=>m.id===e);if(!r||!se(r))return;let o=ie(r).find(m=>m.id===a);if(!o){v("Choose a valid option.");return}!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&(t.marketTradeStatus="Loading position...",g(),await F(),t.marketTradeStatus=null);let n=t.marketSnapshots[r.id];if(We(r,n)){v("This market is resolved and can no longer be traded.");return}let s=t.marketPositions[r.id],i=t.marketOrderMode==="sell";if(!i&&s?.optionId){v("Your pick is already locked for this market.");return}if(i&&!s?.optionId){v("You do not have a pick to exit.");return}let d=Math.max(0,Number(s?.optionSharesUsdc)||0);if(i&&d<=0){v("Your pick is still loading. Please try again."),await F({force:!0});return}let u=i?d:Ie(Number(t.marketTradeAmount)||0,"buy","yes",void 0);t.marketTradeAmount=u,t.marketTradeOptionId=i&&s?.optionId||o.id,C("trade_attempt");try{t.marketTradeStatus=i?"Exiting your pick...":"Locking your pick...",g(),await ga(r.id,i?"sell":"buy",i&&s?.optionId||o.id,u,m=>{t.marketTradeStatus=m,g()}),delete t.marketSnapshots[r.id],delete t.marketPositions[r.id],delete t.checkedMarketSnapshots[r.id],t.hasLoadedPortfolioPositions=!1,t.portfolioPositionsLoadedAt=0,t.walletAddress=await Ne(),t.walletAddress&&(t.walletBalance=await te(t.walletAddress)),await F({force:!0}),C(i?"trade_sell_success":"trade_buy_success"),v(i?"Pick exited":`Pick locked: ${o.label}`),t.tradeDrawerOpen=!1}catch(m){C("trade_failed"),v(m instanceof Error?m.message:"Trade failed")}finally{t.marketTradeStatus=null,B(),g()}},hr=()=>{if(!T||!k)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(k.hidden=!0,T.hidden=!1,T.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){T.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){T.innerHTML=`
  ...
  `},br=e=>{let a=t.marketSnapshots[e.id],r=G(e),o=se(e),n=ie(e).length,s=a?.yesPriceCents,i=s??e.probability,d=o?`${n}`:`${i}%`,u=s===void 0?r?"Loading Arc pools":"Arc setup required":`Yes ${s}\xA2 \xB7 No ${100-s}\xA2`,m=s===void 0?`Yes ${e.probability}c - No ${100-e.probability}c`:u,f=qe(e),p=e.timeframe==="Daily"?Ft(e,a):e.closes;return`
...
Trade and discuss here: ${d}`},kr=e=>{if(!k||!T)return;let a=qe(e),r=!t.checkedMarketEvidence[e.id],o=G(e),n=t.marketSnapshots[e.id],s=se(e),i=ie(e);s&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let d=Ga(e),u=!!(o&&!n),m=n?.yesPriceCents??(o?e.probability:0),f=n?.noPriceCents??(o?100-e.probability:0),p=u?"":o?`${m}\xA2`:"--",h=u?"":o?`${f}\xA2`:"--",l=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},c=!!l.optionId;s&&c&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),s&&!c&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let b=s&&t.marketOrderMode==="sell"&&c?Math.max(0,Number(l.optionSharesUsdc)||0):0,x=b>0?b:Ie(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,l),w=b>0?{min:0,max:b}:Qt(t.marketOrderMode,t.marketTradeSide,l),$=t.marketOrderMode==="buy"?"$5-$10 USDC":`Up to $${O(w.max)} USDC`,y=!t.walletAddress||t.hasLoadedPortfolioPositions,S=We(e,n),P=qt(e,n),U=!!P;s||(t.marketTradeSide=nt(t.marketOrderMode,t.marketTradeSide,l));let N=!s&&!S&&!U&&y&&ee(t.marketOrderMode,"yes",l),R=!s&&!S&&!U&&y&&ee(t.marketOrderMode,"no",l),de=s?!S&&!U&&y&&(t.marketOrderMode==="sell"?c:!c&&!!d):!S&&!U&&y&&ee(t.marketOrderMode,t.marketTradeSide,l),_=S?"Market resolved":P||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),ce=S?"Market resolved":P||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),pe=s?x:Xt(n,t.marketTradeSide,x,t.marketOrderMode,l),ue=t.marketOrderMode==="buy"?"Buy":"Exit",V=s?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";k.hidden=!0,T.hidden=!1,T.classList.add("fullscreen"),document.body.classList.add("detail-mode"),rr(e),Vt(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&F({force:!t.hasLoadedPortfolioPositions});let Be=s?!!l.optionId:l.yesSharesUsdc>0||l.noSharesUsdc>0,ye="";if(s&&Be&&t.walletAddress){let A=n?.optionPools?.[l.optionId||""]||0,D=n?.volumeUsdc||0,Z=l.optionSharesUsdc&&A>0?l.optionSharesUsdc/A*D:0;ye=`
        ...
        <div class="market-detail-main">
          ...
          <section class="market-evidence-thread">
            ...
            <div class="market-thread-timeline">
              ${r?lr(3):a.evidence.length===0?'<div class="portfolio-empty compact">Related news is still loading for this market.</div>':a.evidence.map(A=>{let D=zt(e,A),Z=t.unlockingSummaryUrl===A.sourceUrl;return`
                <article class="market-thread-update">
                  ...
                  <div class="market-thread-update-content">
                    ...
                    ${ze(D)}
                    ${Z?`<div style="margin-top: 12px;">${he()}</div>`:oe(D)?t.loadingSummaryUrl===A.sourceUrl?`<div style="margin-top: 12px;">${he()}</div>`:`<div style="margin-top: 12px;">${rt(K(D,t.aiSummaries[A.sourceUrl]))}</div>`:""}
                  </div>
                </article>
              `}).join("")}
            </div>
      ...
      <div class="trade-drawer ${t.tradeDrawerOpen?"open":""}" id="tradeDrawer">
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${s?i.map(A=>{let D=n?.optionPools?.[A.id]||0,Z=t.marketTradeOptionId===A.id||l.optionId===A.id,dt=S||U||t.marketOrderMode==="sell"||c||!y;return`
  ...
  `;let a=(l="2026-07-19T23:59:59.000Z")=>{let c=document.getElementById("seasonTimer");J&&clearInterval(J);let b=()=>{let w=new Date(l).getTime()-new Date().getTime();if(w<=0){c&&(c.innerText="Season Finished!"),J&&clearInterval(J);return}let $=Math.floor(w/(1e3*60*60*24)),y=Math.floor(w%(1e3*60*60*24)/(1e3*60*60)),S=Math.floor(w%(1e3*60*60)/(1e3*60)),P=Math.floor(w%(1e3*60)/1e3);c&&(c.innerText=`${$}d ${y}h ${S}m ${P}s`)};b(),J=setInterval(b,1e3)};a();let r=l=>l.map((c,b)=>{let x=Number(c.globalRank)||b+1,w=String(c.username||""),$=!!(t.walletAddress&&w.toLowerCase()===t.walletAddress.toLowerCase()),y=$&&t.profileUsername?t.profileUsername:c.displayName||w,S=$?`${t.profileUsername?y:j(w)} (You)`:y.startsWith("0x")&&y.length===42?j(y):y,P=M(S),U=M(Ve(c.status)),N=c.nextSeasonDivision?`Division ${c.nextSeasonDivision}`:"Qualify",R=x<=10?"promotion-zone":"safety-zone",de=x<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
    ...
    `)},i=()=>{n("global");let l=document.getElementById("leaderboardListContainer"),c="global",b=vt(c);b&&l?(l.innerHTML=yt()+r(b.players),b.seasonEndsAt&&a(b.seasonEndsAt)):s(10);let x=new URLSearchParams;t.walletAddress&&x.set("walletAddress",t.walletAddress);let w=x.toString();fetch(E(`/api/leaderboard/global${w?`?${w}`:""}`)).then($=>$.json()).then($=>{let y=bt($.players||[],b?.players||[],!0);l&&(l.innerHTML=y.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:r(y)),Ze(c,{players:y,seasonEndsAt:$.seasonEndsAt}),a($.seasonEndsAt)}).catch($=>{console.error("Failed to load global leaderboard:",$),l&&(l.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},d=l=>{n("division");let c=document.getElementById("leaderboardListContainer"),b=`division_${l||ke||"current"}`,x=vt(b);x&&c?(c.innerHTML=yt()+o(x.players),x.divisionNumber&&(ke=x.divisionNumber),x.seasonEndsAt&&a(x.seasonEndsAt)):c&&l!==void 0&&(c.innerHTML=`
      ...
      `);let w=new URLSearchParams;t.walletAddress&&w.set("walletAddress",t.walletAddress),l&&w.set("division",String(l));let $=w.toString();fetch(E(`/api/leaderboard/division${$?`?${$}`:""}`)).then(y=>y.json()).then(y=>{let S=y.divisionNumber||1,P=bt(y.players||[],x?.players||[],!1),U=y.totalDivisions||1,N=y.seasonEndsAt;ke=S;let R=document.getElementById("divisionTitleText");R&&(R.innerText=`Division ${S}`);let de=document.getElementById("divisionSelector");de&&(de.innerHTML=Array.from({length:U},(_,ce)=>ce+1).map(_=>`
            <option value="${_}" ${_===S?"selected":""}>Division ${_}</option>
          `).join("")),c&&(P.length===0?c.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No active players in this division.</p>`:c.innerHTML=P.map((_,ce)=>{let pe=ce+1,ue=t.walletAddress&&_.username.toLowerCase()===t.walletAddress.toLowerCase(),V=ue&&t.profileUsername?t.profileUsername:_.displayName||_.username,lt=M(Ve(_.status)),Be=ue?`${t.profileUsername?V:j(_.username)} (You)`:V.startsWith("0x")&&V.length===42?j(V):V,ye=M(Be),A="safety-zone",D='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return pe<=2?(A="promotion-zone",D='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):pe>=5&&(A="relegation-zone",D='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
    ...
    `}let o=wr(r?.outcome),n=ea(a,r),s=n.reduce((c,b)=>Math.max(c,b.payout),0),i=a.yesSharesUsdc+a.noSharesUsdc,d=r?.outcome??0,u=_e().has(e.id),m=d===1?a.yesSharesUsdc:d===2?a.noSharesUsdc:0,f=d===1?r?.yesSharesUsdc??0:d===2?r?.noSharesUsdc??0:0,p=r?.volumeUsdc??0,h=m>0&&f>0?m/f*p:0,l=d===0?"":u?'<span style="color: #34d399; font-size: 0.82rem; font-weight: 800;">Claimed</span>':h>0?`<button type="button" class="connect-wallet-btn" data-claim-market="${e.id}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 6px !important; padding: 8px 14px !important; font-size: 0.82rem !important; font-weight: 700 !important; cursor: pointer !important;">Claim $${O(h)}</button>`:'<span style="color: #ef4444; font-size: 0.82rem; font-weight: 800;">Lost</span>';return`
    <article class="portfolio-position-card">
      <div class="portfolio-position-top">
        <span class="category-chip ${e.category}">${z(e.category)}</span>
        <span>${o}</span>
  ...
  `},Sr=async e=>{if(!t.walletAddress){v("Please sign in first.");return}let a=Me().find(o=>o.id===e),r=a?G(a):"";if(!a||!r){v("Market is not available.");return}try{C("claim_attempt"),it();let o=await ha(r,t.walletAddress);C("claim_success"),o.won&&Za(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await te(t.walletAddress),await F(),v(o.won?`Claimed $${O(o.amountUsdc)}`:"No payout to claim"),B(),W()}catch(o){C("claim_failed"),v(o instanceof Error?o.message:"Claim failed")}},xr=e=>{if(!e)return"";let a=t.referralData,r=a?.referrals?.length?a.referrals.map(n=>{let s=n.displayName||j(n.walletAddress),i=n.remaining<=0;return`

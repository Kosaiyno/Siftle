...
`,ot=e=>{let a=e.type==="tweet",r='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: text-top; color: var(--color-text-primary);"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',o='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 12px; height: 12px; display: inline-block; vertical-align: text-top; margin-right: 4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';return`
  ...
  `},ke=()=>{if(!S)return;if(S.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){S.innerHTML=Sr(4);return}if(te){S.innerHTML=jr(),ie();return}let e=x(t.newsSearchQuery.trim()),r=`
...
`},Ir=async(e,a)=>{if(!t.walletAddress){w("Session expired or wallet not connected. Please sign in."),Ie();return}let r=Le().find(p=>p.id===e);if(!r||!oe(r))return;let o=ne(r).find(p=>p.id===a);if(!o){w("Choose a valid option.");return}!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&(t.marketTradeStatus="Loading position...",g(),await z(),t.marketTradeStatus=null);let s=t.marketSnapshots[r.id];if(Ke(r,s)){w("This market is resolved and can no longer be traded.");return}let n=t.marketPositions[r.id],i=t.marketOrderMode==="sell";if(!i&&n?.optionId){w("Your pick is already locked for this market.");return}if(i&&!n?.optionId){w("You do not have a pick to exit.");return}let l=Math.max(0,Number(n?.optionSharesUsdc)||0);if(i&&l<=0){w("Your pick is still loading. Please try again."),await z({force:!0});return}let c=i?l:De(Number(t.marketTradeAmount)||0,"buy","yes",void 0);t.marketTradeAmount=c,t.marketTradeOptionId=i&&n?.optionId||o.id,U("trade_attempt");try{t.marketTradeStatus=i?"Exiting your pick...":"Locking your pick...",g(),await Sa(r.id,i?"sell":"buy",i&&n?.optionId||o.id,c,p=>{t.marketTradeStatus=p,g()}),delete t.marketSnapshots[r.id],delete t.marketPositions[r.id],delete t.checkedMarketSnapshots[r.id],t.hasLoadedPortfolioPositions=!1,t.portfolioPositionsLoadedAt=0,t.walletAddress=await je(),t.walletAddress&&(t.walletBalance=await ce(t.walletAddress)),await z({force:!0}),U(i?"trade_sell_success":"trade_buy_success"),w(i?"Pick exited":`Pick locked: ${o.label}`),t.tradeDrawerOpen=!1}catch(p){U("trade_failed"),w(p instanceof Error?p.message:"Trade failed")}finally{t.marketTradeStatus=null,_(),g()}},Er=()=>{if(!M||!S)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(S.hidden=!0,M.hidden=!1,M.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){M.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){M.innerHTML=`
  ...
  `},Br=()=>{if(!M||!S)return;if(t.selectedThreadUrl){Er();return}let e=t.stories.find(i=>i.id===t.selectedStoryId);if(!e){M.hidden=!0,M.classList.remove("fullscreen"),document.body.classList.remove("detail-mode"),S.hidden=!1;return}if(e.type==="tweet"){S.hidden=!0,M.hidden=!1,M.classList.add("fullscreen"),document.body.classList.add("detail-mode");let i='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 16px; height: 16px; display: inline-block; vertical-align: text-top; margin-right: 6px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';M.innerHTML=`
    ...
    `;return}let a=ue(e,t.aiSummaries[e.sourceUrl]),r=t.loadingSummaryUrl===e.sourceUrl,o=me(e),s=t.unlockingSummaryUrl===e.sourceUrl,n=vt(e);S.hidden=!0,M.hidden=!1,M.classList.add("fullscreen"),document.body.classList.add("detail-mode"),M.innerHTML=`
        ...
        <div style="display: flex; flex-direction: column; gap: 6px;">
          ${d.evidence.slice(0,2).map(h=>`
            <div style="display: flex; align-items: flex-start; gap: 6px; font-size: 0.76rem; text-align: left; line-height: 1.35; padding: 4px 0;">
              <span style="background: rgba(59, 130, 246, 0.08); color: var(--market-accent); border: 1px solid rgba(59, 130, 246, 0.15); border-radius: 4px; padding: 1px 4px; font-size: 0.62rem; font-weight: 700; text-transform: uppercase; flex-shrink: 0; line-height: 1;">${x(h.source)}</span>
              <span style="color: var(--market-text-main); font-weight: 500;">${x(h.headline)}</span>
            </div>
          `).join("")}
        </div>
      </div>
...
Trade and discuss here: ${l}`},Nr=e=>{if(!S||!M)return;let a=Je(e),r=!t.checkedMarketEvidence[e.id],o=K(e),s=t.marketSnapshots[e.id],n=oe(e),i=ne(e);n&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let l=nr(e),c=!!(o&&!s),p=s?.yesPriceCents??(o?e.probability:0),u=s?.noPriceCents??(o?100-e.probability:0),d=c?"":o?`${p}\xA2`:"--",v=c?"":o?`${u}\xA2`:"--",h=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},k=!!h.optionId;n&&k&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),n&&!k&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let y=n&&t.marketOrderMode==="sell"&&k?Math.max(0,Number(h.optionSharesUsdc)||0):0,T=y>0?y:De(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,h),f=y>0?{min:0,max:y}:na(t.marketOrderMode,t.marketTradeSide,h),m=t.marketOrderMode==="buy"?"exactly $2.00 USDC":`Up to $${H(f.max)} USDC`,L=!t.walletAddress||t.hasLoadedPortfolioPositions,b=Ke(e,s),$=Qt(e,s),A=!!$;n||(t.marketTradeSide=kt(t.marketOrderMode,t.marketTradeSide,h));let P=!n&&!b&&!A&&L&&de(t.marketOrderMode,"yes",h),N=!n&&!b&&!A&&L&&de(t.marketOrderMode,"no",h),Y=n?!b&&!A&&L&&(t.marketOrderMode==="sell"?k:!k&&!!l):!b&&!A&&L&&de(t.marketOrderMode,t.marketTradeSide,h),q=b?"Market resolved":$||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),V=b?"Market resolved":$||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),Z=n?T:ia(s,t.marketTradeSide,T,t.marketOrderMode,h),Q=t.marketOrderMode==="buy"?"Buy":"Exit",ge=n?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";S.hidden=!0,M.hidden=!1,M.classList.add("fullscreen"),document.body.classList.add("detail-mode"),gr(e),oa(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&z({force:!t.hasLoadedPortfolioPositions});let St=n?!!h.optionId:h.yesSharesUsdc>0||h.noSharesUsdc>0,Ze="";n&&St&&t.walletAddress?Ze=`
      <div class="user-market-position-box" style="margin: 16px 0; padding: 16px; background: rgba(59, 130, 246, 0.05); border: 1px solid rgba(59, 130, 246, 0.15); border-radius: 12px; font-family: 'Space Grotesk', sans-serif;">
        ...
        <div style="display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 12px;">
          <div>
            ...
            <strong style="font-size: 0.95rem; color: var(--market-text-main);">${x(h.optionLabel||"Selected option")}</strong>
          </div>
          <div>
    ...
    <div class="detail-container market-detail-container">
      ...
      <div class="sticky-trade-bar">
        ...
        <button class="sticky-trade-btn" type="button" id="openTradeDrawerBtn" ${b||A?"disabled":""}>
          ${b?"Market Resolved":$||(n?k?"Pick Locked":"Pick Outcome":"Trade Market")}
        </button>
      </div>
      ...
      <div class="trade-drawer ${t.tradeDrawerOpen?"open":""}" id="tradeDrawer">
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${n?i.map(B=>{let O=t.marketTradeOptionId===B.id||h.optionId===B.id,he=b||A||t.marketOrderMode==="sell"||k||!L;return`
                  <button type="button" class="market-side option ${O?"active":""} ${he?"disabled":""}" data-market-option-id="${x(B.id)}" ${he?"disabled":""}>
                    <span>${x(B.label)}</span>
          ...
          <div class="market-amount-panel">
            ...
            <div class="market-amount-input-row">
              <span>$</span>
              <input id="marketAmountInput" type="number" min="${f.min.toFixed(2)}" max="${Math.max(f.min,f.max).toFixed(2)}" step="0.01" inputmode="decimal" value="${T}" data-market-amount ${b||A||t.marketOrderMode==="buy"?"disabled":""} style="${t.marketOrderMode==="buy"?"opacity: 0.7; cursor: not-allowed;":""}" />
              <span>USDC</span>
            </div>
          </div>
  ...
  `},fa=()=>{if(!S||!M)return;Ye?.toggleAttribute("hidden",!0),Ge?.toggleAttribute("hidden",!0),ae?.toggleAttribute("hidden",!0),Pe?.classList.remove("active"),Ue?.classList.remove("active"),Ce?.classList.remove("active"),document.body.classList.remove("detail-mode"),M.hidden=!0,S.hidden=!1,S.classList.add("markets-list");let e=t.walletAddress&&t.hasLoadedPortfolioPositions?wt():null;t.walletAddress&&e&&fetch(I("/api/leaderboard/report"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({walletAddress:t.walletAddress,points:e.points,status:e.status,username:t.profileUsername||""})}).catch(f=>console.error("Failed to report user score:",f)),ee&&(clearInterval(ee),ee=null),S.innerHTML=`
  ...
  `,((f="2026-07-19T23:59:59.000Z")=>{let m=document.getElementById("seasonTimer");ee&&clearInterval(ee);let L=()=>{let $=new Date(f).getTime()-new Date().getTime();if($<=0){m&&(m.innerText="Season Finished!"),ee&&clearInterval(ee);return}let A=Math.floor($/(1e3*60*60*24)),P=Math.floor($%(1e3*60*60*24)/(1e3*60*60)),N=Math.floor($%(1e3*60*60)/(1e3*60)),Y=Math.floor($%(1e3*60)/1e3);m&&(m.innerText=`${A}d ${P}h ${N}m ${Y}s`)};L(),ee=setInterval(L,1e3)})();let r=f=>f.map((m,L)=>{let b=Number(m.globalRank)||L+1,$=String(m.username||""),A=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),P=A&&t.profileUsername?t.profileUsername:m.displayName||$,N=A?`${t.profileUsername?P:R($)} (You)`:P.startsWith("0x")&&P.length===42?R(P):P,Y=x(N),q=Ut(m.status);if(m.totalTrades!==void 0&&m.aiBriefingUnlocks!==void 0){let Ve=Ct(m.status);q=`${Ve.wins}W - ${Ve.losses}L \xB7 ${m.totalTrades} trades \xB7 ${m.aiBriefingUnlocks} unlocks`}let V=x(q),Z=m.nextSeasonDivision?`Division ${m.nextSeasonDivision}`:"Qualify",Q=b<=10?"promotion-zone":"safety-zone",ge=b<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
      <div class="leaderboard-row global-row ${A?"user-highlight":""} ${Q}" role="listitem">
        ...
        <div class="leaderboard-row-score">
          ...
          <span>${m.prizeEligible?"Prize eligible":"Season rank"} \xB7 ${x(Z)}</span>
        </div>
        <div class="leaderboard-row-right">
          <span>${V}</span>
        </div>
      </div>
    `}).join(""),o=f=>f.map((m,L)=>{let b=L+1,$=String(m.username||""),A=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),P=A&&t.profileUsername?t.profileUsername:m.displayName||$,N=Ut(m.status);if(m.totalTrades!==void 0&&m.aiBriefingUnlocks!==void 0){let ge=Ct(m.status);N=`${ge.wins}W - ${ge.losses}L \xB7 ${m.totalTrades} trades \xB7 ${m.aiBriefingUnlocks} unlocks`}let Y=x(N),q=A?`${t.profileUsername?P:R($)} (You)`:P.startsWith("0x")&&P.length===42?R(P):P,V=x(q),Z="safety-zone",Q='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return b<=2?(Z="promotion-zone",Q='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):b>=5&&(Z="relegation-zone",Q='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
    ...
    `}).join(""),s=f=>f.map((m,L)=>{let b=L+1,$=String(m.username||""),A=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),P=A&&t.profileUsername?t.profileUsername:m.displayName||$,N=A?`${t.profileUsername?P:R($)} (You)`:P.startsWith("0x")&&P.length===42?R(P):P,Y=x(N),q=Number(m.unlocks)||0,V=Number(m.points)||0,Z=m.status||`${q} briefing unlock${q===1?"":"s"}`;return`
    ...
    `)},i=()=>{let f=document.getElementById("leaderboardListContainer");n(f,6),fetch(I("/api/leaderboard/preseason")).then(m=>m.json()).then(m=>{let L=m.players||[];f&&(f.innerHTML=L.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the preseason leaderboard yet. Unlock a daily AI briefing to join!</p>`:s(L))}).catch(m=>{console.error("Failed to load preseason leaderboard:",m),f&&(f.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading preseason leaderboard. Please try again.</p>`)})},l=()=>{let f=document.getElementById("season1LeaderboardListContainer");n(f,6),fetch(I("/api/leaderboard/season1")).then(m=>m.json()).then(m=>{let L=m.map((b,$)=>{let A=$+1,P=null;return A<=6?P=1:A<=12&&(P=2),{username:b.wallet_address,displayName:b.username,points:b.points,status:`${b.wins} wins, ${b.losses} losses`,totalTrades:b.total_trades,aiBriefingUnlocks:b.ai_briefing_unlocks,globalRank:A,prizeEligible:A<=10,nextSeasonDivision:P}});if(f)if(G==="global")f.innerHTML=L.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players in Season 1.</p>`:r(L);else{let b=document.getElementById("season1DivisionSelector"),$=b?Number(b.value):1,A=L.filter(P=>P.nextSeasonDivision===$);f.innerHTML=A.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players in this division.</p>`:o(A)}}).catch(m=>{console.error("Failed to load Season 1 archive:",m),f&&(f.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading Season 1 leaderboard archive. Please try again.</p>`)})},c=f=>{G=f,document.querySelectorAll("[data-season1-view]").forEach(m=>{m.classList.toggle("active",m.dataset.season1View===f)}),document.getElementById("season1DivisionControls")?.toggleAttribute("hidden",f!=="division"),document.getElementById("season1GlobalControls")?.toggleAttribute("hidden",f!=="global"),document.getElementById("season1PrizeBox")?.toggleAttribute("hidden",f!=="global"),l()};i(),X&&c(G);let p=document.getElementById("archiveExpandBtn"),u=document.getElementById("archiveContent"),d=document.getElementById("archiveChevron");p?.addEventListener("click",()=>{X=!X,u&&(u.style.display=X?"block":"none"),d&&(d.style.transform=X?"rotate(180deg)":"rotate(0deg)"),X&&c(G)}),document.querySelectorAll("[data-season1-view]").forEach(f=>{f.addEventListener("click",()=>{let m=f.dataset.season1View==="division"?"division":"global";c(m)})}),document.getElementById("season1DivisionSelector")?.addEventListener("change",()=>{l()}),document.getElementById("faucetClaimButton")?.addEventListener("click",async()=>{if(!t.walletAddress){w("Please sign in first!");return}if(localStorage.getItem("siftle_circle_is_mock")==="true"){let m=parseFloat(localStorage.getItem(`siftle_mock_balance_${t.walletAddress}`)||"1000.00")+100;localStorage.setItem(`siftle_mock_balance_${t.walletAddress}`,m.toFixed(2)),t.walletBalance=m.toFixed(2),w("Claimed $100 USDC mock credits!"),_(),fa()}else w("Opening Circle Faucet..."),window.open(st,"_blank")});let k=document.getElementById("howItWorksBtn"),y=document.getElementById("howItWorksModal"),T=document.getElementById("closeRulesModalBtn");k?.addEventListener("click",()=>{y&&y.classList.add("active")}),T?.addEventListener("click",()=>{y&&y.classList.remove("active")}),y?.addEventListener("click",f=>{f.target===y&&y.classList.remove("active")})},ga=()=>{t.activeSurface="feed",t.selectedMarketId=null,Ye?.toggleAttribute("hidden",!0),Ge?.toggleAttribute("hidden",!0),ae?.toggleAttribute("hidden",!0),Pe?.classList.remove("active"),Ue?.classList.add("active"),Ce?.classList.remove("active"),S?.classList.remove("markets-list")},Or=e=>e===1?"Yes resolved":e===2?"No resolved":e===3?"Invalid":"Open",Dt=e=>{let a=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},r=t.marketSnapshots[e.id];if(oe(e)){let k=r?.resolvedOptionId||null,y=!!k,T=y&&a.optionId===k,f=ur(a,r),m=T?f:0,L=ne(e).find(A=>A.id===k)?.label,b=!!a.claimedAt||qe().has(e.id),$=!!t.claimingMarketIds[e.id];return`
      <article class="portfolio-position-card">
        <div class="portfolio-position-top">
          <span class="category-chip ${e.category}">${W(e.category)}</span>
          <span>${y?`Resolved: ${x(L||"Option selected")}`:"Open"}</span>
        </div>
        <h2>${e.question}</h2>
        <div class="portfolio-position-stats">
          <div><span>Your pick</span><strong>${x(a.optionLabel||"Selected option")}</strong></div>
    ...
    `}let o=Or(r?.outcome),s=la(a,r),n=s.reduce((k,y)=>Math.max(k,y.payout),0),i=a.yesSharesUsdc+a.noSharesUsdc,l=r?.outcome??0,c=qe().has(e.id),p=l===1?a.yesSharesUsdc:l===2?a.noSharesUsdc:0,u=l===1?r?.yesSharesUsdc??0:l===2?r?.noSharesUsdc??0:0,d=r?.volumeUsdc??0,v=p>0&&u>0?p/u*d:0,h=l===0?"":c?'<span style="color: #34d399; font-size: 0.82rem; font-weight: 800;">Claimed</span>':v>0?`<button type="button" class="connect-wallet-btn" data-claim-market="${e.id}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 6px !important; padding: 8px 14px !important; font-size: 0.82rem !important; font-weight: 700 !important; cursor: pointer !important;">Claim $${H(v)}</button>`:'<span style="color: #ef4444; font-size: 0.82rem; font-weight: 800;">Lost</span>';return`
    <article class="portfolio-position-card">
      <div class="portfolio-position-top">
        <span class="category-chip ${e.category}">${W(e.category)}</span>
        <span>${o}</span>
  ...
  `},Hr=async e=>{if(!t.walletAddress){w("Please sign in first.");return}let a=Le().find(o=>o.id===e),r=a?K(a):"";if(!a||!r){w("Market is not available.");return}try{t.claimingMarketIds[a.id]=!0,j(),U("claim_attempt"),wt();let o=await xa(r,t.walletAddress);U("claim_success"),o.won&&cr(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ce(t.walletAddress),await z(),w(o.won?`Claimed $${H(o.amountUsdc)}`:"No payout to claim"),_(),j()}catch(o){U("claim_failed"),w(o instanceof Error?o.message:"Claim failed")}finally{delete t.claimingMarketIds[a.id],j()}},Rr=e=>{if(!e)return"";let a=t.referralData,r=a?.referrals?.length?a.referrals.map(s=>{let n=s.displayName||R(s.walletAddress),i=s.remaining<=0;return`
      ...
      `}).join(""):'<div class="portfolio-empty compact">No referrals yet.</div>',o=t.loadingReferralData&&!a?'<div class="portfolio-referral-message">Loading invite tools...</div>':t.referralError&&!a?`
        <div class="portfolio-referral-message">
          <span>${x(t.referralError)}</span>
          <button type="button" data-refresh-referrals>Retry</button>
        </div>
      `:a?`
          <div class="portfolio-referral-copy-grid">
            <button type="button" class="portfolio-referral-copy" data-copy-referral-code="${x(a.code)}">
              <span>Invite code</span>
              <strong>${x(a.code)}</strong>
            </button>
            <button type="button" class="portfolio-referral-copy" data-copy-referral-link="${x(a.inviteLink)}">
  ...
  `},j=()=>{if(!S||!M)return;Ye?.toggleAttribute("hidden",!0),Ge?.toggleAttribute("hidden",!0),ae?.toggleAttribute("hidden",!0),Pe?.classList.remove("active"),Ue?.classList.remove("active"),Ce?.classList.add("active"),document.body.classList.remove("detail-mode"),M.hidden=!0,S.hidden=!1,S.classList.add("markets-list"),t.walletAddress&&!t.referralData&&!t.referralError&&!t.loadingReferralData&&Me(),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&(t.portfolioMarketPreviews.length===0&&Wt(),z({force:!t.hasLoadedPortfolioPositions}));let a=qe(),r=Le().filter(d=>{let v=t.marketPositions[d.id];return a.has(d.id)||v&&(v.yesSharesUsdc+v.noSharesUsdc>0||(v.optionSharesUsdc||0)>0)}),o=r.filter(d=>(t.marketSnapshots[d.id]?.outcome??0)===0),s=r.filter(d=>(t.marketSnapshots[d.id]?.outcome??0)!==0),n=!!t.walletAddress,i=t.profileUsername||(t.walletAddress?R(t.walletAddress):"Anonymous"),l=x(i),c=x(t.profileUsername||""),p=t.profileNotice?`<div style="margin-top: 14px !important; padding: 10px 12px !important; border-radius: 8px !important; border: 1px solid ${t.profileNotice.type==="error"?"rgba(239, 68, 68, 0.28)":"rgba(16, 185, 129, 0.24)"} !important; background: ${t.profileNotice.type==="error"?"rgba(127, 29, 29, 0.22)":"rgba(6, 95, 70, 0.18)"} !important; color: ${t.profileNotice.type==="error"?"#fca5a5":"#86efac"} !important; font-size: 0.8rem !important; font-weight: 650 !important;">${x(t.profileNotice.message)}</div>`:"",u=i.charAt(0).toUpperCase();S.innerHTML=`
    <section class="portfolio-surface">
      ...
      ${t.loadingPortfolioPositions?Tr(2):t.walletAddress?r.length===0?'<div class="portfolio-empty">No positions found for this wallet yet. Confirmed trades will appear here after the Arc transaction settles.</div>':`
...
`),s="",n="",i=!1,l=!1;for(let c=0;c<o.length;c++){let p=o[c].trim();if(!p)continue;if(/what matters/i.test(p)||p.includes("\u{1F3AF}")){i&&(n+="</ul></div>",i=!1);let v=p.replace(/^#+\s*/,"").replace(/[🎯⚡⏱️⭐]\s*/g,"");n+=`
  ...
  `,document.body.appendChild(a);let r=()=>a.remove();a.querySelector("#prefCloseBtn")?.addEventListener("click",r),a.addEventListener("click",o=>{o.target===a&&r()}),a.querySelector("#prefClearBtn")?.addEventListener("click",()=>{a.querySelector("#clubInput").value="",a.querySelector("#managerInput").value="",a.querySelector("#playerInput").value=""}),a.querySelector("#prefSaveBtn")?.addEventListener("click",()=>{let o=a.querySelector("#clubInput")?.value||"",s=a.querySelector("#managerInput")?.value||"",n=a.querySelector("#playerInput")?.value||"",i=c=>c.split(",").map(p=>p.trim()).filter(Boolean),l={clubs:i(o),managers:i(s),players:i(n)};zr(l),w("Topics saved"),r(),t.activeCategory="Personalized",g()})},jr=()=>{if(ze)return`

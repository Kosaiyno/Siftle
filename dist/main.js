...
`,ue=()=>{if(!y)return;let e=ma();if(y.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){y.innerHTML=_a(4);return}if(e.length===0){let a=t.showSaved?[]:t.stories;if(a.length>0){y.innerHTML=a.map(r=>`
  ...
  <article class="thread-item">
    ...
    <div class="thread-item-body">
      ...
      ${ee(e)?t.loadingSummaryUrl===e.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${je(q(e,t.aiSummaries[e.sourceUrl]||e.ai_summary))}</div>`:""}
    </div>
  </article>
`,Ya=()=>{if(!k||!y)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(y.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){k.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){k.innerHTML=`
    ...
    <div class="detail-container thread-container">
      ...
      <article class="detail-card thread-card">
        ...
        <div class="thread-timeline">
          ${mt(e,"Latest")}
          ${Ut(r?.items??[]).map(o=>mt(o,o.postedAt||"Earlier")).join("")}
        </div>
      </article>
    </div>
  ...
  `},Ja=e=>{let a=t.marketSnapshots[e.id],r=j(e),o=a?.yesPriceCents,s=o??e.probability,i=`${s}%`,n=o===void 0?r?"Loading Arc pools":"Arc setup required":`Yes ${o}\xA2 \xB7 No ${100-o}\xA2`,m=o===void 0?`Yes ${e.probability}c - No ${100-e.probability}c`:n,d=Ae(e);return`
  ...
  `},Ka=e=>{let a=Ae(e),o=t.marketSnapshots[e.id]?.yesPriceCents??e.probability,s=100-o,i=a.evidence[0],n=i?i.headline:"No updates yet",m=`${window.location.origin}${window.location.pathname}#market-${e.id}`;return`\u{1F6A8} *Siftle Market Alert* \u{1F6A8}
...
Trade and discuss here: ${m}`},Va=e=>{if(!y||!k)return;let a=Ae(e),r=!t.checkedMarketEvidence[e.id],o=j(e),s=t.marketSnapshots[e.id],i=!!(o&&!s),n=s?.yesPriceCents??(o?e.probability:0),m=s?.noPriceCents??(o?100-e.probability:0),d=i?"":o?`${n}\xA2`:"--",c=i?"":o?`${m}\xA2`:"--",f=qe(Number(t.marketTradeAmount)||0),u=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},p=!t.walletAddress||t.hasLoadedPortfolioPositions,l=Ye(e,s),g=Tt(e,s),v=!!g;t.marketTradeSide=Ge(t.marketOrderMode,t.marketTradeSide,u);let w=!l&&!v&&p&&Z(t.marketOrderMode,"yes",u),x=!l&&!v&&p&&Z(t.marketOrderMode,"no",u),$=!l&&!v&&p&&Z(t.marketOrderMode,t.marketTradeSide,u),N=l?"Market resolved":g||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),P=l?"Market resolved":g||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),re=It(s,t.marketTradeSide,f,t.marketOrderMode,u),Y=t.marketOrderMode==="buy"?"Buy":"Exit",C=o?"Arc testnet live":"Contract not deployed";y.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),Ba(e),Pt(e),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&z();let G=u.yesSharesUsdc>0||u.noSharesUsdc>0,J="";G&&t.walletAddress&&(J=`
      ...
      <article class="market-detail-card">
        <div class="market-detail-main">
          <div class="market-detail-topline">
            <span class="category-chip ${e.category}">${B(e.category)}</span>
            <span class="market-status-pill">${C}</span>
          </div>
          <h2>${e.question}</h2>
          ${J}
          ${a.imageUrl?`
          ...
          <div class="market-stats-row">
            <div class="market-stat">
              <span>${ze(e,s)===null?"Closes":"Trade lock"}</span>
                ...
                <article class="market-thread-update">
                  ...
                  <div class="market-thread-update-content">
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
                      ...
                      <button type="button" class="market-thread-source-link" data-unlock-briefing-url="${encodeURIComponent(A.sourceUrl)}">${ee(we(e,A))?"AI briefing":"Unlock AI briefing"}</button>
                    </div>
                    ${ee(we(e,A))?t.loadingSummaryUrl===A.sourceUrl?`<div style="margin-top: 12px;">${Ke()}</div>`:`<div style="margin-top: 12px;">${je(q(we(e,A),t.aiSummaries[A.sourceUrl]))}</div>`:""}
                  </div>
                </article>
              `).join("")}
            </div>
          ...
          <div class="market-action-grid">
            ${i?`
              ...
              `:`
                ...
                <button type="button" class="market-side no ${t.marketTradeSide==="no"?"active":""} ${x?"":"disabled"}" data-market-trade-side="no" ${x?"":"disabled"} title="${x?"No":P}">
                  <span>No</span>
                  <strong>${c}</strong>
                  ${x?"":`<small>${P}</small>`}
                </button>
  ...
  `},Ht=()=>{if(!y||!k)return;if(xe?.toggleAttribute("hidden",!0),$e?.toggleAttribute("hidden",!0),X?.toggleAttribute("hidden",!0),fe?.classList.add("active"),ge?.classList.remove("active"),he?.classList.remove("active"),window.setTimeout(()=>{t.activeSurface==="markets"&&M.forEach(i=>{Pt(i)})},750),t.selectedMarketId){let i=M.find(n=>n.id===t.selectedMarketId);if(i){Va(i);return}t.selectedMarketId=null,window.location.hash.startsWith("#market-")&&window.history.replaceState({},"","#markets");return}document.body.classList.remove("detail-mode"),k.hidden=!0,k.classList.remove("fullscreen"),y.hidden=!1,y.classList.add("markets-list");let e=M,r=`
  ...
  `},Rt=()=>{if(!y||!k)return;xe?.toggleAttribute("hidden",!0),$e?.toggleAttribute("hidden",!0),X?.toggleAttribute("hidden",!0),fe?.classList.remove("active"),ge?.classList.remove("active"),he?.classList.remove("active"),document.body.classList.remove("detail-mode"),k.hidden=!0,y.hidden=!1,y.classList.add("markets-list");let e=t.walletAddress&&t.hasLoadedPortfolioPositions?Je():null;t.walletAddress&&e&&fetch(T("/api/leaderboard/report"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({walletAddress:t.walletAddress,points:e.points,status:e.status,username:t.profileUsername||""})}).catch(p=>console.error("Failed to report user score:",p)),W&&(clearInterval(W),W=null),y.innerHTML=`
  ...
  `;let a=(p="2026-07-19T23:59:59.000Z")=>{let l=document.getElementById("seasonTimer");W&&clearInterval(W);let g=()=>{let w=new Date(p).getTime()-new Date().getTime();if(w<=0){l&&(l.innerText="Season Finished!"),W&&clearInterval(W);return}let x=Math.floor(w/(1e3*60*60*24)),$=Math.floor(w%(1e3*60*60*24)/(1e3*60*60)),N=Math.floor(w%(1e3*60*60)/(1e3*60)),P=Math.floor(w%(1e3*60)/1e3);l&&(l.innerText=`${x}d ${$}h ${N}m ${P}s`)};g(),W=setInterval(g,1e3)};a();let r=p=>p.map((l,g)=>{let v=Number(l.globalRank)||g+1,w=String(l.username||""),x=!!(t.walletAddress&&w.toLowerCase()===t.walletAddress.toLowerCase()),$=x&&t.profileUsername?t.profileUsername:l.displayName||w,N=x?`${t.profileUsername?$:_(w)} (You)`:$.startsWith("0x")&&$.length===42?_($):$,P=I(N),re=I(at(l.status)),Y=l.nextSeasonDivision?`Division ${l.nextSeasonDivision}`:"Qualify",C=v<=10?"promotion-zone":"safety-zone",G=v<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
      <div class="leaderboard-row global-row ${x?"user-highlight":""} ${C}" role="listitem">
        ...
        <div class="leaderboard-row-score">
          ...
          <span>${l.prizeEligible?"Prize eligible":"Season rank"} \xB7 ${I(Y)}</span>
        </div>
    ...
    `)},i=()=>{o("global"),s(10);let p=document.getElementById("leaderboardListContainer"),l=new URLSearchParams;t.walletAddress&&l.set("walletAddress",t.walletAddress);let g=l.toString();fetch(T(`/api/leaderboard/global${g?`?${g}`:""}`)).then(v=>v.json()).then(v=>{let w=v.players||[];p&&(p.innerHTML=w.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:r(w)),a(v.seasonEndsAt)}).catch(v=>{console.error("Failed to load global leaderboard:",v),p&&(p.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},n=p=>{o("division");let l=document.getElementById("leaderboardListContainer");l&&p!==void 0&&(l.innerHTML=`
      ...
      `);let g=new URLSearchParams;t.walletAddress&&g.set("walletAddress",t.walletAddress),p&&g.set("division",String(p));let v=g.toString();fetch(T(`/api/leaderboard/division${v?`?${v}`:""}`)).then(w=>w.json()).then(w=>{let x=w.divisionNumber||1,$=w.players||[],N=w.totalDivisions||1,P=w.seasonEndsAt;Ue=x;let re=document.getElementById("divisionTitleText");re&&(re.innerText=`Division ${x}`);let Y=document.getElementById("divisionSelector");Y&&(Y.innerHTML=Array.from({length:N},(C,G)=>G+1).map(C=>`
            <option value="${C}" ${C===x?"selected":""}>Division ${C}</option>
          `).join("")),l&&($.length===0?l.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No active players in this division.</p>`:l.innerHTML=$.map((C,G)=>{let J=G+1,A=t.walletAddress&&C.username.toLowerCase()===t.walletAddress.toLowerCase(),O=A&&t.profileUsername?t.profileUsername:C.displayName||C.username,zt=I(at(C.status)),Wt=A?`${t.profileUsername?O:_(C.username)} (You)`:O.startsWith("0x")&&O.length===42?_(O):O,qt=I(Wt),Me="safety-zone",Le='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return J<=2?(Me="promotion-zone",Le='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):J>=5&&(Me="relegation-zone",Le='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
                <div class="leaderboard-row ${A?"user-highlight":""} ${Me}" role="listitem" style="display: flex !important; align-items: center !important; justify-content: space-between !important; padding: 12px 16px !important; border-bottom: 1px solid rgba(255, 255, 255, 0.06) !important; margin-bottom: 0 !important; background: transparent !important; font-family: 'Space Grotesk', sans-serif !important;">
                  ...
                  <div style="flex: 1; display: flex; align-items: center; justify-content: center;">
                    <span style="color: #ffffff; font-weight: 750; font-size: 0.95rem; white-space: nowrap;">${C.points} pts</span>
                  </div>
                  <!-- Right Side: Status -->
              ...
              `}).join("")),a(P)}).catch(w=>{console.error("Failed to load division leaderboard:",w),l&&(l.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading division leaderboard. Please try again.</p>`)})};K==="division"?n(Ue||void 0):i(),document.querySelectorAll("[data-leaderboard-view]").forEach(p=>{p.addEventListener("click",()=>{(p.dataset.leaderboardView==="division"?"division":"global")==="division"?n(Ue||void 0):i()})}),document.getElementById("divisionSelector")?.addEventListener("change",p=>{let l=Number(p.target.value);n(l)}),document.getElementById("faucetClaimButton")?.addEventListener("click",async()=>{if(!t.walletAddress){b("Please sign in first!");return}if(localStorage.getItem("siftle_circle_is_mock")==="true"){let l=parseFloat(localStorage.getItem(`siftle_mock_balance_${t.walletAddress}`)||"1000.00")+100;localStorage.setItem(`siftle_mock_balance_${t.walletAddress}`,l.toFixed(2)),t.walletBalance=l.toFixed(2),b("Claimed $100 USDC mock credits!"),U(),Rt()}else b("Opening Circle Faucet..."),window.open(Be,"_blank")});let c=document.getElementById("howItWorksBtn"),f=document.getElementById("howItWorksModal"),u=document.getElementById("closeRulesModalBtn");c?.addEventListener("click",()=>{f&&f.classList.add("active")}),u?.addEventListener("click",()=>{f&&f.classList.remove("active")}),f?.addEventListener("click",p=>{p.target===f&&f.classList.remove("active")})},jt=()=>{t.activeSurface="feed",t.selectedMarketId=null,xe?.removeAttribute("hidden"),$e?.removeAttribute("hidden"),X?.removeAttribute("hidden"),fe?.classList.remove("active"),ge?.classList.add("active"),he?.classList.remove("active"),y?.classList.remove("markets-list")},Za=e=>e===1?"Yes resolved":e===2?"No resolved":e===3?"Invalid":"Open",pt=e=>{let a=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},r=t.marketSnapshots[e.id],o=Za(r?.outcome),s=Bt(a,r),i=s.reduce((g,v)=>Math.max(g,v.payout),0),n=a.yesSharesUsdc+a.noSharesUsdc,m=r?.outcome??0,d=Fe().has(e.id),c=m===1?a.yesSharesUsdc:m===2?a.noSharesUsdc:0,f=m===1?r?.yesSharesUsdc??0:m===2?r?.noSharesUsdc??0:0,u=r?.volumeUsdc??0,p=c>0&&f>0?c/f*u:0,l=m===0?"":d?'<span style="color: #34d399; font-size: 0.82rem; font-weight: 800;">Claimed</span>':p>0?`<button type="button" class="connect-wallet-btn" data-claim-market="${e.id}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 6px !important; padding: 8px 14px !important; font-size: 0.82rem !important; font-weight: 700 !important; cursor: pointer !important;">Claim $${H(p)}</button>`:'<span style="color: #ef4444; font-size: 0.82rem; font-weight: 800;">Lost</span>';return`
  ...
  `},Qa=async e=>{if(!t.walletAddress){b("Please sign in first.");return}let a=Se().find(o=>o.id===e),r=a?j(a):"";if(!a||!r){b("Market is not available.");return}try{L("claim_attempt"),Je();let o=await Vt(r,t.walletAddress);L("claim_success"),o.won&&Ua(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ce(t.walletAddress),await z(),b(o.won?`Claimed $${H(o.amountUsdc)}`:"No payout to claim"),U(),F()}catch(o){L("claim_failed"),b(o instanceof Error?o.message:"Claim failed")}},Xa=()=>{if(!t.referralPanelOpen)return"";let e=t.referralData,a=e?.referrals?.length?e.referrals.map(r=>{let o=r.displayName||_(r.walletAddress),s=r.remaining<=0;return`
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 12px 0; border-bottom: 1px solid var(--market-border);">
          <div style="min-width: 0;">
            <strong style="display: block; color: var(--market-text-main); font-size: 0.92rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${I(o)}</strong>
            <span style="color: var(--market-text-muted); font-size: 0.76rem;">${_(r.walletAddress)}</span>
          </div>
        ...
        ${t.loadingReferralData||!e?'<div class="portfolio-empty compact">Loading referral details...</div>':`
            <div style="display: grid; gap: 12px;">
              <div style="border: 1px solid var(--market-border); border-radius: 10px; padding: 14px; background: var(--market-bg);">
                ...
                <strong style="display: block; color: var(--market-text-main); font-size: 1.25rem; margin-top: 4px;">${I(e.code)}</strong>
              </div>
              <button type="button" class="connect-wallet-btn" data-copy-referral-link="${I(e.inviteLink)}" style="background: #ffffff !important; color: #000000 !important; border: 1px solid #ffffff !important; border-radius: 8px !important; padding: 10px 14px !important; font-weight: 800 !important;">Copy invite link</button>
  ...
  `},F=()=>{if(!y||!k)return;xe?.toggleAttribute("hidden",!0),$e?.toggleAttribute("hidden",!0),X?.toggleAttribute("hidden",!0),fe?.classList.remove("active"),ge?.classList.remove("active"),he?.classList.add("active"),document.body.classList.remove("detail-mode"),k.hidden=!0,y.hidden=!1,y.classList.add("markets-list"),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&(t.portfolioMarketPreviews.length===0&&bt(),t.referralPanelOpen&&!t.referralData&&!t.loadingReferralData&&He(),z());let e=Fe(),a=Se().filter(f=>{let u=t.marketPositions[f.id];return e.has(f.id)||u&&u.yesSharesUsdc+u.noSharesUsdc>0}),r=a.filter(f=>(t.marketSnapshots[f.id]?.outcome??0)===0),o=a.filter(f=>(t.marketSnapshots[f.id]?.outcome??0)!==0),s=!!t.walletAddress,i=t.profileUsername||(t.walletAddress?_(t.walletAddress):"Anonymous"),n=I(i),m=I(t.profileUsername||""),d=t.profileNotice?`<div style="margin-top: 14px !important; padding: 10px 12px !important; border-radius: 8px !important; border: 1px solid ${t.profileNotice.type==="error"?"rgba(239, 68, 68, 0.28)":"rgba(16, 185, 129, 0.24)"} !important; background: ${t.profileNotice.type==="error"?"rgba(127, 29, 29, 0.22)":"rgba(6, 95, 70, 0.18)"} !important; color: ${t.profileNotice.type==="error"?"#fca5a5":"#86efac"} !important; font-size: 0.8rem !important; font-weight: 650 !important;">${I(t.profileNotice.message)}</div>`:"",c=i.charAt(0).toUpperCase();y.innerHTML=`
  ...
  `},h=()=>{if(kt.forEach(e=>{let a=e.dataset.bottomNav;e.classList.toggle("active",a==="saved"?t.showSaved:a===t.activeSurface&&!t.showSaved)}),t.activeSurface==="markets"){Ht();return}if(t.activeSurface==="portfolio"){F();return}if(t.activeSurface==="leaderboard"){Rt();return}jt(),Lt(),ue(),Ga(),D&&(D.value=t.activeArchiveDate??"")};Ne.textContent=St();X?.addEventListener("click",e=>{let r=e.target.closest("[data-category]");r&&(t.activeCategory=r.dataset.category,window.history.pushState({},"","#feed"),R(),h(),be(),te(t.activeCategory))});fe?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="markets",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,window.history.pushState({},"","#markets"),R(),h()});ge?.addEventListener("click",()=>{t.activeSurface="feed",t.showSaved=!1,window.history.pushState({},"","#feed"),R(),h(),be(),te(t.activeCategory)});he?.addEventListener("click",()=>{t.feedScrollY=window.scrollY,t.activeSurface="portfolio",t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=!1,window.history.pushState({},"","#portfolio"),R(),h()});V?.addEventListener("click",()=>{t.walletAddress?(window.location.hash="#portfolio",ve()):Re()});document.addEventListener("click",e=>{let a=e.target,r=a.closest(".copy-address-btn");if(r){let i=r.getAttribute("data-address");i&&navigator.clipboard.writeText(i).then(()=>{b("Wallet address copied!")})}let o=a.closest("[data-claim-market]");if(o){let i=o.getAttribute("data-claim-market");i&&Qa(i);return}if(a.closest("[data-open-referrals]")){t.referralPanelOpen=!0,He(),F();return}if(a.closest("[data-close-referrals]")){t.referralPanelOpen=!1,F();return}let s=a.closest("[data-copy-referral-link]");if(s){let i=s.getAttribute("data-copy-referral-link")||"";i&&navigator.clipboard.writeText(i).then(()=>b("Invite link copied"));return}a.closest("[data-connect-wallet]")&&(t.walletAddress?gt():Re())});kt.forEach(e=>{e.addEventListener("click",()=>{let a=e.dataset.bottomNav;t.selectedMarketId=null,t.selectedStoryId=null,t.selectedThreadUrl=null,t.showSaved=a==="saved",a==="markets"?(t.activeSurface="markets",window.history.pushState({},"","#markets")):a==="portfolio"?(t.activeSurface="portfolio",window.history.pushState({},"","#portfolio")):a==="leaderboard"?(t.activeSurface="leaderboard",window.history.pushState({},"","#leaderboard")):(t.activeSurface="feed",window.history.pushState({},"","#feed"),be(),te(t.activeCategory),a==="saved"&&(La(),Oe(),pe())),R(),h()})});D?.addEventListener("change",()=>{t.activeArchiveDate=D.value||null,window.history.pushState({},"","#feed"),R(),h(),te(t.activeCategory)});na?.addEventListener("click",()=>{t.activeArchiveDate=null,D&&(D.value=""),window.history.pushState({},"","#feed"),R(),h(),te(t.activeCategory)});y?.addEventListener("click",async e=>{let a=e.target;if(a.closest("#editUsernameBtn")){let p=y?.querySelector(".username-display-row"),l=y?.querySelector("#usernameEditForm");if(p&&l){p.style.display="none",l.style.display="flex";let g=l.querySelector("#usernameInput");g&&g.focus()}return}if(a.closest("#cancelUsernameBtn")){let p=y?.querySelector(".username-display-row"),l=y?.querySelector("#usernameEditForm");p&&l&&(p.style.display="flex",l.style.display="none");return}let s=a.closest("#saveUsernameBtn");if(s){let l=y?.querySelector("#usernameEditForm")?.querySelector("#usernameInput");if(l){let g=l.value.trim().slice(0,15),v=s,w=v.textContent||"Save";v.disabled=!0,v.textContent="Saving...",Ma(g),t.profileNotice=null;try{t.walletAddress&&await ae(!1),t.profileNotice={type:"success",message:"Username saved to your shared leaderboard profile."},b("Username updated"),F()}catch(x){let $=x instanceof Error?x.message:"Username save failed";t.profileNotice={type:"error",message:$},b($),v.disabled=!1,v.textContent=w,F()}}return}let i=a.closest("[data-timeframe]");if(i){let p=i.dataset.timeframe;t.activeMarketTimeframe=p,Ht();return}let n=a.closest("[data-market-id]");if(n){t.selectedMarketId=n.dataset.marketId??null,L("market_view"),window.history.pushState({},"",`#market-${t.selectedMarketId}`),h(),window.scrollTo({top:0,behavior:"smooth"});return}let m=a.closest("[data-thread-story-id]"),d=a.closest("[data-export-id]"),c=a.closest("[data-export-action]"),f=a.closest("[data-story-id]");if(m){e.stopPropagation();let p=t.stories.find(l=>l.id===Number(m.dataset.threadStoryId));p&&va(p);return}let u=a.closest(".mobile-bookmark-btn, .bookmark-button");if(u){e.stopPropagation();let p=u.dataset.bookmarkUrl||"",l=t.stories.find(g=>g.sourceUrl===p);if(!l)return;l.saved=!l.saved,l.saved?Q.add(p):Q.delete(p),ia(),b(l.saved?"Saved to your list":"Removed from saved"),ue();return}if(c){e.stopPropagation(),Fa(Number(c.dataset.exportStoryId),c.dataset.exportAction);return}if(d){e.stopPropagation();let p=Number(d.dataset.exportId);t.activeShareStoryId=t.activeShareStoryId===p?null:p,ue();return}f&&(a.closest("a")||At(Number(f.dataset.storyId)))});y?.addEventListener("keydown",e=>{let r=e.target.closest("[data-story-id]");!r||e.key!=="Enter"&&e.key!==" "||(e.preventDefault(),At(Number(r.dataset.storyId)))});k?.addEventListener("click",e=>{let a=e.target,r=a.closest("[data-unlock-briefing]");if(r){let d=t.stories.find(c=>c.id===Number(r.dataset.unlockBriefing));d&&tt(d);return}let o=a.closest("[data-unlock-briefing-url]");if(o){let d=decodeURIComponent(o.dataset.unlockBriefingUrl||""),c=fa(d);c&&(ee(c)?Te(c):tt(c));return}if(a.closest("[data-back-markets]")){t.selectedMarketId=null,t.tradeDrawerOpen=!1,window.history.pushState({},"","#markets"),h();return}if(a.closest("#openTradeDrawerBtn")){let d=M.find(u=>u.id===t.selectedMarketId);if(d){if(Ye(d,t.marketSnapshots[d.id])){b("This market is resolved and can no longer be traded.");return}if(Tt(d,t.marketSnapshots[d.id])){b("Trading is locked 20 minutes before kickoff.");return}}t.tradeDrawerOpen=!0,L("trade_drawer_open");let c=k.querySelector("#tradeDrawer"),f=k.querySelector("#tradeDrawerBackdrop");c?.classList.add("open"),f?.classList.add("open");return}if(a.closest("#closeTradeDrawerBtn")||a.closest("#tradeDrawerBackdrop")){t.tradeDrawerOpen=!1;let d=k.querySelector("#tradeDrawer"),c=k.querySelector("#tradeDrawerBackdrop");d?.classList.remove("open"),c?.classList.remove("open");return}if(a.closest("#shareWhatsAppBtn")){let d=M.find(c=>c.id===t.selectedMarketId);if(d){let c=Ka(d),f=`https://api.whatsapp.com/send?text=${encodeURIComponent(c)}`;window.open(f,"_blank")}return}let s=a.closest("[data-market-trade]");if(s&&t.selectedMarketId){let d=s.dataset.marketTrade;Da(t.selectedMarketId,d);return}let i=a.closest("[data-market-trade-side]");if(i){if(i.disabled||i.classList.contains("disabled"))return;let d=M.find(u=>u.id===t.selectedMarketId),c=d?t.marketPositions[d.id]:void 0,f=i.dataset.marketTradeSide;if(!Z(t.marketOrderMode,f,c))return;t.marketTradeSide=f,h();return}let n=a.closest("[data-market-order-mode]");if(n){t.marketOrderMode=n.dataset.marketOrderMode;let d=M.find(f=>f.id===t.selectedMarketId),c=d?t.marketPositions[d.id]:void 0;t.marketTradeSide=Ge(t.marketOrderMode,t.marketTradeSide,c),h();return}a.closest("[data-back-to-feed]")&&ba()});k?.addEventListener("input",e=>{let a=e.target;if(!a.matches("[data-market-amount]"))return;t.marketTradeAmount=qe(Number(a.value)||0);let r=M.find(m=>m.id===t.selectedMarketId),o=r?t.marketSnapshots[r.id]:void 0,s=r?t.marketPositions[r.id]:void 0,i=It(o,t.marketTradeSide,t.marketTradeAmount,t.marketOrderMode,s),n=k.querySelector(".market-inline-payout strong");n&&(n.textContent=`$${H(i)}`)});k?.addEventListener("focusin",e=>{e.target.matches("[data-market-amount]")&&document.body.classList.add("market-amount-focused")});k?.addEventListener("focusout",e=>{e.target.matches("[data-market-amount]")&&window.setTimeout(()=>document.body.classList.remove("market-amount-focused"),120)});window.addEventListener("popstate",ve);window.addEventListener("hashchange",ve);Pe?.addEventListener("click",()=>{if(!Ee||!Pe)return;let e=!Ee.hidden;Ee.hidden=e,Pe.setAttribute("aria-expanded",String(!e))});document.addEventListener("click",e=>{let a=e.target;!a.closest(".share-control")&&t.activeShareStoryId!==null&&(t.activeShareStoryId=null,ue());let r=a.closest("[data-menu-action]");if(!r)return;let o={today:"Showing today's brief",saved:`${t.stories.filter(s=>s.saved).length} saved stories`,archive:t.archiveDates.length>0?"Choose a saved day from the archive selector":"No saved days yet"};S&&(S.textContent=o[r.dataset.menuAction??"today"]),r.dataset.menuAction==="today"&&(t.showSaved=!1,t.activeArchiveDate=null,D&&(D.value=""),R(),be(),te(t.activeCategory)),r.dataset.menuAction==="saved"&&(jt(),Oe(),pe(),t.showSaved=!0,document.querySelector("#archiveControls")?.classList.remove("mobile-open"),R(),h())});h();U();sa().then(()=>{Ia(),h(),U(),window.setTimeout(ar,1200),wa()});var er=document.querySelector("#mobileArchiveCard"),se=document.querySelector("#archiveControls");er?.addEventListener("click",()=>{if(!se)return;se.classList.toggle("mobile-open")&&setTimeout(()=>se.scrollIntoView({behavior:"smooth",block:"center"}),50)});var tr=document.querySelector("#archivePill");tr?.addEventListener("click",e=>{if(e.stopPropagation(),!se)return;se.classList.toggle("mobile-open")&&setTimeout(()=>se.scrollIntoView({behavior:"smooth",block:"center"}),50)});var ke=!1,ut=!1,ar=()=>{ut||(ut=!0,(async()=>{let e=await De();if(ke=!!e,e){t.walletConnecting=!0,U();try{let a=await Zt();ke=!1,t.walletConnecting=!1,a?(t.walletAddress=await De(),t.walletAddress&&(oe(),t.walletBalance=await ce(t.walletAddress),await z()),U(),t.activeSurface==="portfolio"&&h()):(t.walletAddress=null,t.walletBalance=null,oe(),b("Session expired. Please sign in again."),U(),h())}catch(a){console.warn(a),ke=!1,t.walletConnecting=!1,t.walletAddress=null,t.walletBalance=null,oe(),b("Session expired. Please sign in again."),U(),h()}}await Qt(a=>{ke||(t.walletAddress=a,t.walletBalance=null,oe(),a&&ae(!1).catch(r=>console.error("Failed to report leaderboard entry:",r)),t.marketPositions={},t.hasLoadedPortfolioPositions=!1,U(),a?(ce(a).then(r=>{t.walletBalance=r,U(),t.activeSurface==="portfolio"&&h()}),z()):t.activeSurface==="portfolio"&&h())})})())};L("app_open");document.addEventListener("click",e=>{let r=e.target.closest("a, button");if(r){let o=r.className||"",s=typeof o=="string"?o:r.getAttribute("class")||"",i=r.getAttribute("href")||"";!(r.hasAttribute("data-unlock-briefing")||r.hasAttribute("data-unlock-briefing-url"))&&(s.includes("source-button")||s.includes("source-btn")||s.includes("source-link")||r.textContent?.trim()==="Open source")&&!s.includes("disabled")&&i!=="#"&&L("open_source")}},!0);

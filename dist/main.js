...
const loadStorySummary = async (story) => {
    if (state.aiSummaries[story.sourceUrl] || state.loadingSummaryUrl === story.sourceUrl)
        return;
    if (story.ai_summary) {
        state.aiSummaries[story.sourceUrl] = story.ai_summary;
        if (menuStatus) {
    ...
    try {
        ...
        state.aiSummaries[story.sourceUrl] = data.summary || story.summary;
        ...
        console.warn(error);
        state.aiSummaries[story.sourceUrl] = story.summary;
        if (menuStatus) {
...
const renderDetail = () => {
    ...
    const summary = state.aiSummaries[story.sourceUrl] || story.summary;

...
`;var tt=e=>{let a=e.type==="tweet",r='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: text-top; color: var(--color-text-primary);"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',o='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 12px; height: 12px; display: inline-block; vertical-align: text-top; margin-right: 4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';return`
  ...
  `;if(t.activeCategory==="Personalized"){let i=Ue(),s=ma(),n=[...i.clubs,...i.players,...i.managers].join(", ");if(!s){x.innerHTML=`
...
`},Pr=async(e,a)=>{if(!t.walletAddress){k("Session expired or wallet not connected. Please sign in."),Ae();return}let r=Pe().find(p=>p.id===e);if(!r||!Qt(r))return;let o=Zt(r).find(p=>p.id===a);if(!o){k("Choose a valid option.");return}!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&(t.marketTradeStatus="Loading position...",y(),await q(),t.marketTradeStatus=null);let i=t.marketSnapshots[r.id];if(ft(r,i)){k("This market is resolved and can no longer be traded.");return}let s=t.marketPositions[r.id],n=t.marketOrderMode==="sell";if(!n&&s?.optionId){k("Your pick is already locked for this market.");return}if(n&&!s?.optionId){k("You do not have a pick to exit.");return}let d=Math.max(0,Number(s?.optionSharesUsdc)||0);if(n&&d<=0){k("Your pick is still loading. Please try again."),await q({force:!0});return}let l=n?d:Ke(Number(t.marketTradeAmount)||0,"buy","yes",void 0);t.marketTradeAmount=l,t.marketTradeOptionId=n&&s?.optionId||o.id,U("trade_attempt");try{t.marketTradeStatus=n?"Exiting your pick...":"Locking your pick...",y(),await at(r.id,n?"sell":"buy",n&&s?.optionId||o.id,l,p=>{t.marketTradeStatus=p,y()}),delete t.marketSnapshots[r.id],delete t.marketPositions[r.id],delete t.checkedMarketSnapshots[r.id],t.hasLoadedPortfolioPositions=!1,t.portfolioPositionsLoadedAt=0,t.walletAddress=await je(),t.walletAddress&&(t.walletBalance=await re(t.walletAddress)),await q({force:!0}),U(n?"trade_sell_success":"trade_buy_success"),k(n?"Pick exited":`Pick locked: ${o.label}`),t.tradeDrawerOpen=!1}catch(p){U("trade_failed"),k(p instanceof Error?p.message:"Trade failed")}finally{t.marketTradeStatus=null,O(),y()}},Ar=()=>{if(!A||!x)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(x.hidden=!0,A.hidden=!1,A.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){A.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){A.innerHTML=`
  ...
  `};var Er=e=>{let a=Xt(e),o=t.marketSnapshots[e.id]?.yesPriceCents??e.probability,i=100-o,s=a.evidence[0],n=s?s.headline:"No updates yet",d=`${window.location.origin}${window.location.pathname}#market-${e.id}`;return`\u{1F6A8} *Siftle Market Alert* \u{1F6A8}
              ...
              ${(()=>{let m=t.marketPositions[i.id],h=m&&(m.optionSharesUsdc||m.yesSharesUsdc||0)>0,g=m?.optionId||m?.side,v=dt(i),f=(b,L,$)=>{let S=h&&g===b,M=h&&g!==b;return S?`
                  ...
                  ${h?`
                    <div style="margin-top: 10px; font-size: 0.78rem; color: var(--muted); font-weight: 600; text-align: center; background: rgba(255, 255, 255, 0.03); border-radius: 8px; padding: 8px 12px; border: 1px solid rgba(255, 255, 255, 0.06);">
                      You hold a position in <strong style="color: #34d399;">${w(m.optionLabel||g)}</strong>. You can buy more or sell. Other outcomes are locked.
                    </div>
                  `:""}
                `})()}

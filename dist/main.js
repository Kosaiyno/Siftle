...
Trade and discuss here: ${l}`},Ua=e=>{if(!S||!$)return;let r=ze(e),a=!t.checkedMarketEvidence[e.id],o=K(e),s=t.marketSnapshots[e.id],n=te(e),i=ae(e);n&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let l=Zr(e),c=!!(o&&!s),m=s?.yesPriceCents??(o?e.probability:0),p=s?.noPriceCents??(o?100-e.probability:0),d=c?"":o?`${m}\xA2`:"--",b=c?"":o?`${p}\xA2`:"--",f=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},k=!!f.optionId;n&&k&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),n&&!k&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let v=n&&t.marketOrderMode==="sell"&&k?Math.max(0,Number(f.optionSharesUsdc)||0):0,A=v>0?v:Ee(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,f),h=v>0?{min:0,max:v}:Qt(t.marketOrderMode,t.marketTradeSide,f),u=t.marketOrderMode==="buy"?"exactly $2.00 USDC":`Up to $${H(h.max)} USDC`,U=!t.walletAddress||t.hasLoadedPortfolioPositions,y=je(e,s),x=jt(e,s),T=!!x;n||(t.marketTradeSide=dt(t.marketOrderMode,t.marketTradeSide,f));let M=!n&&!y&&!T&&U&&se(t.marketOrderMode,"yes",f),N=!n&&!y&&!T&&U&&se(t.marketOrderMode,"no",f),Y=n?!y&&!T&&U&&(t.marketOrderMode==="sell"?k:!k&&!!l):!y&&!T&&U&&se(t.marketOrderMode,t.marketTradeSide,f),q=y?"Market resolved":x||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),V=y?"Market resolved":x||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),Q=n?A:Xt(s,t.marketTradeSide,A,t.marketOrderMode,f),X=t.marketOrderMode==="buy"?"Buy":"Exit",ue=n?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";S.hidden=!0,$.hidden=!1,$.classList.add("fullscreen"),document.body.classList.add("detail-mode"),la(e),Kt(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&z({force:!t.hasLoadedPortfolioPositions});let pt=n?!!f.optionId:f.yesSharesUsdc>0||f.noSharesUsdc>0,We="";n&&pt&&t.walletAddress?We=`
      ...
      <div class="trade-drawer ${t.tradeDrawerOpen?"open":""}" id="tradeDrawer">
        ...
        <div class="trade-drawer-body">
          ...
          <div class="market-action-grid">
            ${n?i.map(B=>{let O=t.marketTradeOptionId===B.id||f.optionId===B.id,me=y||T||t.marketOrderMode==="sell"||k||!U;return`
          ...
          <div class="market-amount-panel">
            <label for="marketAmountInput">Trade Amount <span style="color: var(--market-text-muted); font-size: 0.72rem; text-transform: none; letter-spacing: 0;">${u}</span></label>
  ...
  `,((h="2026-07-19T23:59:59.000Z")=>{let u=document.getElementById("seasonTimer");ee&&clearInterval(ee);let U=()=>{let x=new Date(h).getTime()-new Date().getTime();if(x<=0){u&&(u.innerText="Season Finished!"),ee&&clearInterval(ee);return}let T=Math.floor(x/(1e3*60*60*24)),M=Math.floor(x%(1e3*60*60*24)/(1e3*60*60)),N=Math.floor(x%(1e3*60*60)/(1e3*60)),Y=Math.floor(x%(1e3*60)/1e3);u&&(u.innerText=`${T}d ${M}h ${N}m ${Y}s`)};U(),ee=setInterval(U,1e3)})();let a=h=>h.map((u,U)=>{let y=Number(u.globalRank)||U+1,x=String(u.username||""),T=!!(t.walletAddress&&x.toLowerCase()===t.walletAddress.toLowerCase()),M=T&&t.profileUsername?t.profileUsername:u.displayName||x,N=T?`${t.profileUsername?M:R(x)} (You)`:M.startsWith("0x")&&M.length===42?R(M):M,Y=C(N),q=yt(u.status);if(u.totalTrades!==void 0&&u.aiBriefingUnlocks!==void 0){let Fe=kt(u.status);q=`${Fe.wins}W - ${Fe.losses}L \xB7 ${u.totalTrades} trades \xB7 ${u.aiBriefingUnlocks} unlocks`}let V=C(q),Q=u.nextSeasonDivision?`Division ${u.nextSeasonDivision}`:"Qualify",X=y<=10?"promotion-zone":"safety-zone",ue=y<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
      <div class="leaderboard-row global-row ${T?"user-highlight":""} ${X}" role="listitem">
        ...
        <div class="leaderboard-row-score">
          ...
          <span>${u.prizeEligible?"Prize eligible":"Season rank"} \xB7 ${C(Q)}</span>
        </div>
        <div class="leaderboard-row-right">
          <span>${V}</span>
        </div>
      </div>
    `}).join(""),o=h=>h.map((u,U)=>{let y=U+1,x=String(u.username||""),T=!!(t.walletAddress&&x.toLowerCase()===t.walletAddress.toLowerCase()),M=T&&t.profileUsername?t.profileUsername:u.displayName||x,N=yt(u.status);if(u.totalTrades!==void 0&&u.aiBriefingUnlocks!==void 0){let ue=kt(u.status);N=`${ue.wins}W - ${ue.losses}L \xB7 ${u.totalTrades} trades \xB7 ${u.aiBriefingUnlocks} unlocks`}let Y=C(N),q=T?`${t.profileUsername?M:R(x)} (You)`:M.startsWith("0x")&&M.length===42?R(M):M,V=C(q),Q="safety-zone",X='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return y<=2?(Q="promotion-zone",X='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):y>=5&&(Q="relegation-zone",X='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
    ...
    `}).join(""),s=h=>h.map((u,U)=>{let y=U+1,x=String(u.username||""),T=!!(t.walletAddress&&x.toLowerCase()===t.walletAddress.toLowerCase()),M=T&&t.profileUsername?t.profileUsername:u.displayName||x,N=T?`${t.profileUsername?M:R(x)} (You)`:M.startsWith("0x")&&M.length===42?R(M):M,Y=C(N),q=Number(u.unlocks)||0,V=Number(u.points)||0,Q=u.status||`${q} briefing unlock${q===1?"":"s"}`;return`
    ...
    `)},i=()=>{let h=document.getElementById("leaderboardListContainer");n(h,6),fetch(I("/api/leaderboard/preseason")).then(u=>u.json()).then(u=>{let U=u.players||[];h&&(h.innerHTML=U.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the preseason leaderboard yet. Unlock a daily AI briefing to join!</p>`:s(U))}).catch(u=>{console.error("Failed to load preseason leaderboard:",u),h&&(h.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading preseason leaderboard. Please try again.</p>`)})},l=()=>{let h=document.getElementById("season1LeaderboardListContainer");n(h,6),fetch(I("/api/leaderboard/season1")).then(u=>u.json()).then(u=>{let U=u.map((y,x)=>{let T=x+1,M=null;return T<=6?M=1:T<=12&&(M=2),{username:y.wallet_address,displayName:y.username,points:y.points,status:`${y.wins} wins, ${y.losses} losses`,totalTrades:y.total_trades,aiBriefingUnlocks:y.ai_briefing_unlocks,globalRank:T,prizeEligible:T<=10,nextSeasonDivision:M}});if(h)if(G==="global")h.innerHTML=U.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players in Season 1.</p>`:a(U);else{let y=document.getElementById("season1DivisionSelector"),x=y?Number(y.value):1,T=U.filter(M=>M.nextSeasonDivision===x);h.innerHTML=T.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players in this division.</p>`:o(T)}}).catch(u=>{console.error("Failed to load Season 1 archive:",u),h&&(h.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading Season 1 leaderboard archive. Please try again.</p>`)})},c=h=>{G=h,document.querySelectorAll("[data-season1-view]").forEach(u=>{u.classList.toggle("active",u.dataset.season1View===h)}),document.getElementById("season1DivisionControls")?.toggleAttribute("hidden",h!=="division"),document.getElementById("season1GlobalControls")?.toggleAttribute("hidden",h!=="global"),document.getElementById("season1PrizeBox")?.toggleAttribute("hidden",h!=="global"),l()};i(),Z&&c(G);let m=document.getElementById("archiveExpandBtn"),p=document.getElementById("archiveContent"),d=document.getElementById("archiveChevron");m?.addEventListener("click",()=>{Z=!Z,p&&(p.style.display=Z?"block":"none"),d&&(d.style.transform=Z?"rotate(180deg)":"rotate(0deg)"),Z&&c(G)}),document.querySelectorAll("[data-season1-view]").forEach(h=>{h.addEventListener("click",()=>{let u=h.dataset.season1View==="division"?"division":"global";c(u)})}),document.getElementById("season1DivisionSelector")?.addEventListener("change",()=>{l()}),document.getElementById("faucetClaimButton")?.addEventListener("click",async()=>{if(!t.walletAddress){w("Please sign in first!");return}if(localStorage.getItem("siftle_circle_is_mock")==="true"){let u=parseFloat(localStorage.getItem(`siftle_mock_balance_${t.walletAddress}`)||"1000.00")+100;localStorage.setItem(`siftle_mock_balance_${t.walletAddress}`,u.toFixed(2)),t.walletBalance=u.toFixed(2),w("Claimed $100 USDC mock credits!"),_(),sr()}else w("Opening Circle Faucet..."),window.open(Ve,"_blank")});let k=document.getElementById("howItWorksBtn"),v=document.getElementById("howItWorksModal"),A=document.getElementById("closeRulesModalBtn");k?.addEventListener("click",()=>{v&&v.classList.add("active")}),A?.addEventListener("click",()=>{v&&v.classList.remove("active")}),v?.addEventListener("click",h=>{h.target===v&&v.classList.remove("active")})},nr=()=>{t.activeSurface="feed",t.selectedMarketId=null,He?.toggleAttribute("hidden",!0),Re?.toggleAttribute("hidden",!0),le?.toggleAttribute("hidden",!0),Me?.classList.remove("active"),Ae?.classList.add("active"),Le?.classList.remove("active"),S?.classList.remove("markets-list")},Pa=e=>e===1?"Yes resolved":e===2?"No resolved":e===3?"Invalid":"Open",At=e=>{let r=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},a=t.marketSnapshots[e.id];if(te(e)){let k=a?.resolvedOptionId||null,v=!!k,A=v&&r.optionId===k,h=sa(r,a),u=A?h:0,U=ae(e).find(T=>T.id===k)?.label,y=!!r.claimedAt||Oe().has(e.id),x=!!t.claimingMarketIds[e.id];return`
      <article class="portfolio-position-card">
        <div class="portfolio-position-top">
          <span class="category-chip ${e.category}">${W(e.category)}</span>
          <span>${v?`Resolved: ${C(U||"Option selected")}`:"Open"}</span>
        </div>
        <h2>${e.question}</h2>
  ...
  `},Ca=async e=>{if(!t.walletAddress){w("Please sign in first.");return}let r=Te().find(o=>o.id===e),a=r?K(r):"";if(!r||!a){w("Market is not available.");return}try{t.claimingMarketIds[r.id]=!0,F(),L("claim_attempt"),ct();let o=await mr(a,t.walletAddress);L("claim_success"),o.won&&aa(r.id),delete t.marketPositions[r.id],delete t.marketSnapshots[r.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ne(t.walletAddress),await z(),w(o.won?`Claimed $${H(o.amountUsdc)}`:"No payout to claim"),_(),F()}catch(o){L("claim_failed"),w(o instanceof Error?o.message:"Claim failed")}finally{delete t.claimingMarketIds[r.id],F()}},Ia=e=>{if(!e)return"";let r=t.referralData,a=r?.referrals?.length?r.referrals.map(s=>{let n=s.displayName||R(s.walletAddress),i=s.remaining<=0;return`

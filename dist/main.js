...
`,Ua=(e=4)=>`${se("Loading stories")}${Array.from({length:e},La).join("")}`,Je=()=>`
  <div class="detail-summary-skeleton" aria-hidden="true">
    ${se("Loading AI briefing")}
...
`,Pa=(e=2)=>`
  <div class="portfolio-skeleton-grid" aria-hidden="true">
    ${se("Loading portfolio positions")}
    ${Array.from({length:e},()=>`
...
`,Oa=()=>{if(!k||!b)return;let e=t.stories.find(o=>o.sourceUrl===t.selectedThreadUrl);if(b.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),!e){k.innerHTML="";return}let a=t.loadingThreadUrl===e.sourceUrl,r=t.activeThread;if(a&&!r){k.innerHTML=`
  ...
  `},Ra=e=>{let a=t.marketSnapshots[e.id],r=F(e),o=a?.yesPriceCents,s=o??e.probability,i=`${s}%`,n=o===void 0?r?"Loading Arc pools":"Arc setup required":`Yes ${o}\xA2 \xB7 No ${100-o}\xA2`,m=o===void 0?`Yes ${e.probability}c - No ${100-e.probability}c`:n,c=$e(e);return`
  ...
  `},Fa=e=>{let a=$e(e),o=t.marketSnapshots[e.id]?.yesPriceCents??e.probability,s=100-o,i=a.evidence[0],n=i?i.headline:"No updates yet",m=`${window.location.origin}${window.location.pathname}#market-${e.id}`;return`\u{1F6A8} *Siftle Market Alert* \u{1F6A8}
...
Trade and discuss here: ${m}`},ja=e=>{if(!b||!k)return;let a=$e(e),r=!t.checkedMarketEvidence[e.id],o=F(e),s=t.marketSnapshots[e.id],i=!!(o&&!s),n=s?.yesPriceCents??(o?e.probability:0),m=s?.noPriceCents??(o?100-e.probability:0),c=i?"":o?`${n}\xA2`:"--",l=i?"":o?`${m}\xA2`:"--",g=qe(Number(t.marketTradeAmount)||0),d=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},u=!t.walletAddress||t.hasLoadedPortfolioPositions,p=ze(e,s),f=xt(e,s),v=!!f;t.marketTradeSide=Ye(t.marketOrderMode,t.marketTradeSide,d);let $=!p&&!v&&u&&V(t.marketOrderMode,"yes",d),x=!p&&!v&&u&&V(t.marketOrderMode,"no",d),A=!p&&!v&&u&&V(t.marketOrderMode,t.marketTradeSide,d),N=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),M=p?"Market resolved":f||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),j=Et(s,t.marketTradeSide,g,t.marketOrderMode,d),I=t.marketOrderMode==="buy"?"Buy":"Exit",te=o?"Arc testnet live":"Contract not deployed";b.hidden=!0,k.hidden=!1,k.classList.add("fullscreen"),document.body.classList.add("detail-mode"),Aa(e),Ut(e),t.walletAddress&&!t.hasLoadedPortfolioPositions&&!t.loadingPortfolioPositions&&H();let L=d.yesSharesUsdc>0||d.noSharesUsdc>0,Y="";L&&t.walletAddress&&(Y=`
      ...
      <article class="market-detail-card">
        <div class="market-detail-main">
          <div class="market-detail-topline">
            <span class="category-chip ${e.category}">${B(e.category)}</span>
            <span class="market-status-pill">${te}</span>
          </div>
          <h2>${e.question}</h2>
          ${Y}
          ${a.imageUrl?`
  ...
  `;let a=d=>d.map((u,p)=>{let f=Number(u.globalRank)||p+1,v=String(u.username||""),$=!!(t.walletAddress&&v.toLowerCase()===t.walletAddress.toLowerCase()),x=$&&t.profileUsername?t.profileUsername:u.displayName||v,A=$?`${t.profileUsername?x:q(v)} (You)`:x.startsWith("0x")&&x.length===42?q(x):x,N=W(A),M=W(et(u.status)),j=u.nextSeasonDivision?`Division ${u.nextSeasonDivision}`:"Qualify",I=f<=10?"promotion-zone":"safety-zone",te=f<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
        ...
        <div class="portfolio-wallet-balance-row" style="margin-top: 24px !important; padding-top: 16px !important; border-top: 1px solid var(--market-border) !important; display: flex !important; justify-content: space-between !important; align-items: center !important; flex-wrap: wrap !important; gap: 12px !important;">
          <div>
            ...
            <strong style="font-size: 1.25rem !important; color: var(--market-text-main) !important; font-family: 'Space Grotesk', sans-serif !important;">
              ${t.walletAddress?t.walletBalance===null?`<span class="skeleton wallet-balance-skeleton" aria-hidden="true" style="display: inline-block !important; width: 80px !important; height: 20px !important; vertical-align: middle !important;"></span>${se("Loading wallet balance")}`:`${t.walletBalance} USDC`:"0.00 USDC"}
            </strong>
          </div>

...
`,Mt=e=>{let a=e.type==="tweet",r='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: text-top; color: var(--color-text-primary);"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',o='<svg class="x-logo-svg" viewBox="0 0 24 24" fill="currentColor" style="width: 12px; height: 12px; display: inline-block; vertical-align: text-top; margin-right: 4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>';return`
  ...
  `},he=()=>{if(!w)return;let e=Oa();if(w.hidden=!!(t.selectedStoryId||t.selectedThreadUrl),t.isLoading){w.innerHTML=kr(4);return}let a=Number(t.unlockConfig?.amountUsdc)||.001,r=M(t.newsSearchQuery.trim()),s=`
    ...
    <button class="market-card" type="button" data-market-id="${e.id}">
      ...
      <div class="market-card-news" style="margin: 12px 0 8px; width: 100%; border-top: 1px dashed var(--market-border); padding-top: 10px; box-sizing: border-box;">
        ...
        <div style="display: flex; flex-direction: column; gap: 6px;">
          ${p.evidence.slice(0,2).map(l=>`
            <div style="display: flex; align-items: flex-start; gap: 6px; font-size: 0.76rem; text-align: left; line-height: 1.35; padding: 4px 0;">
              <span style="background: rgba(59, 130, 246, 0.08); color: var(--market-accent); border: 1px solid rgba(59, 130, 246, 0.15); border-radius: 4px; padding: 1px 4px; font-size: 0.62rem; font-weight: 700; text-transform: uppercase; flex-shrink: 0; line-height: 1;">${M(l.source)}</span>
              <span style="color: var(--market-text-main); font-weight: 500;">${M(l.headline)}</span>
            </div>
          `).join("")}
        </div>
      </div>
...
Trade and discuss here: ${d}`},Br=e=>{if(!w||!S)return;let a=qe(e),r=!t.checkedMarketEvidence[e.id],o=G(e),s=t.marketSnapshots[e.id],n=K(e),i=Z(e);n&&!t.marketTradeOptionId&&(t.marketTradeOptionId=i[0]?.id||null);let d=ar(e),m=!!(o&&!s),u=s?.yesPriceCents??(o?e.probability:0),f=s?.noPriceCents??(o?100-e.probability:0),p=m?"":o?`${u}\xA2`:"--",b=m?"":o?`${f}\xA2`:"--",l=t.marketPositions[e.id]||{yesSharesUsdc:0,noSharesUsdc:0},c=!!l.optionId;n&&c&&t.marketOrderMode!=="sell"&&(t.marketOrderMode="sell"),n&&!c&&t.marketOrderMode==="sell"&&(t.marketOrderMode="buy");let y=n&&t.marketOrderMode==="sell"&&c?Math.max(0,Number(l.optionSharesUsdc)||0):0,k=y>0?y:Be(Number(t.marketTradeAmount)||0,t.marketOrderMode,t.marketTradeSide,l),$=y>0?{min:0,max:y}:ta(t.marketOrderMode,t.marketTradeSide,l),T=t.marketOrderMode==="buy"?"exactly $2.00 USDC":`Up to $${R($.max)} USDC`,v=!t.walletAddress||t.hasLoadedPortfolioPositions,x=We(e,s),L=Yt(e,s),C=!!L;n||(t.marketTradeSide=ut(t.marketOrderMode,t.marketTradeSide,l));let N=!n&&!x&&!C&&v&&te(t.marketOrderMode,"yes",l),H=!n&&!x&&!C&&v&&te(t.marketOrderMode,"no",l),le=n?!x&&!C&&v&&(t.marketOrderMode==="sell"?c:!c&&!!d):!x&&!C&&v&&te(t.marketOrderMode,t.marketTradeSide,l),O=x?"Market resolved":L||(t.marketOrderMode==="sell"?"No YES shares":"Exit NO first"),de=x?"Market resolved":L||(t.marketOrderMode==="sell"?"No NO shares":"Exit YES first"),ye=n?k:aa(s,t.marketTradeSide,k,t.marketOrderMode,l),ce=t.marketOrderMode==="buy"?"Buy":"Exit",Q=n?"Pick one outcome":o?"Arc testnet live":"Contract not deployed";w.hidden=!0,S.hidden=!1,S.classList.add("fullscreen"),document.body.classList.add("detail-mode"),mr(e),Xt(e),t.walletAddress&&(!t.hasLoadedPortfolioPositions||Date.now()-t.portfolioPositionsLoadedAt>15e3)&&!t.loadingPortfolioPositions&&j({force:!t.hasLoadedPortfolioPositions});let De=n?!!l.optionId:l.yesSharesUsdc>0||l.noSharesUsdc>0,be="";n&&De&&t.walletAddress?be=`
    ...
    <div class="detail-container market-detail-container">
      ...
      <div class="sticky-trade-bar">
        ...
        <button class="sticky-trade-btn" type="button" id="openTradeDrawerBtn" ${x||C?"disabled":""}>
      ...
      <div class="trade-drawer ${t.tradeDrawerOpen?"open":""}" id="tradeDrawer">
        ...
        <div class="trade-drawer-body">
          <div class="market-order-mode">
            <button type="button" class="${t.marketOrderMode==="buy"?"active":""}" data-market-order-mode="buy" ${x||C?"disabled":""}>Buy</button>
            <button type="button" class="${t.marketOrderMode==="sell"?"active":""}" data-market-order-mode="sell" ${x||C?"disabled":""}>Exit</button>
          </div>

          <div class="market-action-grid">
            ${n?i.map(U=>{let D=t.marketTradeOptionId===U.id||l.optionId===U.id,pe=x||C||t.marketOrderMode==="sell"||c||!v;return`
          ...
          <div class="market-amount-panel">
            ...
            <div class="market-amount-input-row">
              <span>$</span>
              <input id="marketAmountInput" type="number" min="${$.min.toFixed(2)}" max="${Math.max($.min,$.max).toFixed(2)}" step="0.01" inputmode="decimal" value="${k}" data-market-amount ${x||C||t.marketOrderMode==="buy"?"disabled":""} style="${t.marketOrderMode==="buy"?"opacity: 0.7; cursor: not-allowed;":""}" />
              <span>USDC</span>
            </div>
          </div>
  ...
  `;let a=(l="2026-07-19T23:59:59.000Z")=>{let c=document.getElementById("seasonTimer");J&&clearInterval(J);let y=()=>{let $=new Date(l).getTime()-new Date().getTime();if($<=0){c&&(c.innerText="Season Finished!"),J&&clearInterval(J);return}let T=Math.floor($/(1e3*60*60*24)),v=Math.floor($%(1e3*60*60*24)/(1e3*60*60)),x=Math.floor($%(1e3*60*60)/(1e3*60)),L=Math.floor($%(1e3*60)/1e3);c&&(c.innerText=`${T}d ${v}h ${x}m ${L}s`)};y(),J=setInterval(y,1e3)};a();let r=l=>l.map((c,y)=>{let k=Number(c.globalRank)||y+1,$=String(c.username||""),T=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),v=T&&t.profileUsername?t.profileUsername:c.displayName||$,x=T?`${t.profileUsername?v:z($)} (You)`:v.startsWith("0x")&&v.length===42?z(v):v,L=M(x),C=M(Ve(c.status)),N=c.nextSeasonDivision?`Division ${c.nextSeasonDivision}`:"Qualify",H=k<=10?"promotion-zone":"safety-zone",le=k<=12?'<span class="leaderboard-zone-arrow up">\u25B2</span>':'<span class="leaderboard-zone-arrow invisible">\u2022</span>';return`
    ...
    `}).join(""),o=l=>l.map((c,y)=>{let k=y+1,$=String(c.username||""),T=!!(t.walletAddress&&$.toLowerCase()===t.walletAddress.toLowerCase()),v=T&&t.profileUsername?t.profileUsername:c.displayName||$,x=M(Ve(c.status)),L=T?`${t.profileUsername?v:z($)} (You)`:v.startsWith("0x")&&v.length===42?z(v):v,C=M(L),N="safety-zone",H='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return k<=2?(N="promotion-zone",H='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):k>=5&&(N="relegation-zone",H='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
      <div class="leaderboard-row ${T?"user-highlight":""} ${N}" role="listitem" style="display: flex !important; align-items: center !important; justify-content: space-between !important; padding: 12px 16px !important; border-bottom: 1px solid rgba(255, 255, 255, 0.06) !important; margin-bottom: 0 !important; background: transparent !important; font-family: 'Space Grotesk', sans-serif !important;">
        <div style="flex: 1.5; display: flex; align-items: center; gap: 8px; min-width: 0;">
          ...
          <span class="leaderboard-username" style="font-weight: 600; color: #ffffff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${C}</span>
        </div>
    ...
    `)},i=()=>{s("global");let l=document.getElementById("leaderboardListContainer"),c="global",y=xt(c);y&&l?(l.innerHTML=Tt()+r(y.players),y.seasonEndsAt&&a(y.seasonEndsAt)):n(10);let k=new URLSearchParams;t.walletAddress&&k.set("walletAddress",t.walletAddress);let $=k.toString();fetch(P(`/api/leaderboard/global${$?`?${$}`:""}`)).then(T=>T.json()).then(T=>{let v=$t(T.players||[],y?.players||[],!0);l&&(l.innerHTML=v.length===0?`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No players on the global leaderboard yet.</p>`:r(v)),Ze(c,{players:v,seasonEndsAt:T.seasonEndsAt}),a(T.seasonEndsAt)}).catch(T=>{console.error("Failed to load global leaderboard:",T),l&&(l.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">Error loading global leaderboard. Please try again.</p>`)})},d=l=>{s("division");let c=document.getElementById("leaderboardListContainer"),y=`division_${l||ke||"current"}`,k=xt(y);k&&c?(c.innerHTML=Tt()+o(k.players),k.divisionNumber&&(ke=k.divisionNumber),k.seasonEndsAt&&a(k.seasonEndsAt)):c&&l!==void 0&&(c.innerHTML=`
        <div class="leaderboard-skeleton" style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
          ${Array.from({length:6}).map(()=>`
            <div style="height: 52px; background: rgba(255,255,255,0.02); border: 1px solid #1e1f2b; border-radius: 8px; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0 16px;">
              <div style="display: flex; align-items: center; gap: 12px; width: 60%;">
      ...
      `);let $=new URLSearchParams;t.walletAddress&&$.set("walletAddress",t.walletAddress),l&&$.set("division",String(l));let T=$.toString();fetch(P(`/api/leaderboard/division${T?`?${T}`:""}`)).then(v=>v.json()).then(v=>{let x=v.divisionNumber||1,L=$t(v.players||[],k?.players||[],!1),C=v.totalDivisions||1,N=v.seasonEndsAt;ke=x;let H=document.getElementById("divisionTitleText");H&&(H.innerText=`Division ${x}`);let le=document.getElementById("divisionSelector");le&&(le.innerHTML=Array.from({length:C},(O,de)=>de+1).map(O=>`
            <option value="${O}" ${O===x?"selected":""}>Division ${O}</option>
          `).join("")),c&&(L.length===0?c.innerHTML=`<p style="color: var(--market-text-muted); text-align: center; padding: 24px 0; font-family: 'Space Grotesk', sans-serif;">No active players in this division.</p>`:c.innerHTML=L.map((O,de)=>{let ye=de+1,ce=t.walletAddress&&O.username.toLowerCase()===t.walletAddress.toLowerCase(),Q=ce&&t.profileUsername?t.profileUsername:O.displayName||O.username,ft=M(Ve(O.status)),De=ce?`${t.profileUsername?Q:z(O.username)} (You)`:Q.startsWith("0x")&&Q.length===42?z(Q):Q,be=M(De),U="safety-zone",D='<span style="color: transparent; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u2022</span>';return ye<=2?(U="promotion-zone",D='<span style="color: #34d399; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25B2</span>'):ye>=5&&(U="relegation-zone",D='<span style="color: #ef4444; font-weight: bold; font-size: 0.85rem; margin-right: 4px; display: inline-block; width: 10px;">\u25BC</span>'),`
  ...
  `},Nr=async e=>{if(!t.walletAddress){h("Please sign in first.");return}let a=Me().find(o=>o.id===e),r=a?G(a):"";if(!a||!r){h("Market is not available.");return}try{t.claimingMarketIds[a.id]=!0,q(),E("claim_attempt"),mt();let o=await ba(r,t.walletAddress);E("claim_success"),o.won&&nr(a.id),delete t.marketPositions[a.id],delete t.marketSnapshots[a.id],t.hasLoadedPortfolioPositions=!1,t.walletBalance=await ae(t.walletAddress),await j(),h(o.won?`Claimed $${R(o.amountUsdc)}`:"No payout to claim"),B(),q()}catch(o){E("claim_failed"),h(o instanceof Error?o.message:"Claim failed")}finally{delete t.claimingMarketIds[a.id],q()}},Or=e=>{if(!e)return"";let a=t.referralData,r=a?.referrals?.length?a.referrals.map(s=>{let n=s.displayName||z(s.walletAddress),i=s.remaining<=0;return`

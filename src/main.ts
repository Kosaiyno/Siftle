      ...
      <div class="story-topline desktop-only">
        <div class="story-source">
          <div>
            ...
            ${!isTweet ? `
            <span class="provenance-badge" data-provenance-url="${escapeHtml(story.sourceUrl)}" onclick="event.stopPropagation(); openProvenanceModal(this.dataset.provenanceUrl)" title="Audit source & summary authenticity">
            ...
            <div class="mobile-card-topline">
              ...
              ` : `
                <div class="mobile-source-container">
                  ...
                  <span class="provenance-badge mobile-badge" data-provenance-url="${escapeHtml(story.sourceUrl)}" onclick="event.stopPropagation(); openProvenanceModal(this.dataset.provenanceUrl)" title="Audit source & summary authenticity">
...
// Provenance auditing system
(window as any).openProvenanceModal = async (sourceUrl: string) => {
  // Create or select modal element
  let modal = document.querySelector("#provenanceModal") as HTMLElement;
  if (!modal) {
    modal = document.createElement("div");
    modal.id = "provenanceModal";
  ...
  const closeBtn = modal.querySelector(".provenance-close-btn");
  ...
  try {
    ...
    // Check if summary already exists in Siftle's state
    const hasSummary = story ? Boolean(state.aiSummaries[story.sourceUrl]) : false;
    let summaryProofObj = null;
    if (hasSummary && story) {
      summaryProofObj = state.aiSummaryProofs[story.sourceUrl];
      if (!summaryProofObj) {
        // Mock deterministic inference proof matching standard payload schema
        const hash = tlsProof.data_hash || "local-summary-hash";
        summaryProofObj = {
          providerAddress: "0x61C0007197E7D4d6A842d6768E8035728877B9F6",
          endpoint: "https://router-api.0g.ai/v1",
          model: "deepseek-v4-flash",
          responseId: "mock-cached-summary-resp-" + hash.slice(0, 10),
          status: 200,
          x_0g_proof: "0x09a1" + hash.slice(0, 20) + "d4b2e88a9134c89ea47eb382",
          provenance_bonded: true
        };
      }
    }

    renderProvenanceModalContent(modal, story, tlsProof, summaryProofObj);
  } catch (err: any) {
    modal.innerHTML = `
      <div class="provenance-modal-card">
        <div class="provenance-modal-header">
        ...
        <div class="provenance-modal-body" style="text-align: center; padding: 30px 20px;">
          ...
          <p style="color: #69728a; font-size: 0.8rem; margin-top: 6px;">${escapeHtml(err.message)}</p>
        </div>
      </div>
    `;
    modal.querySelector(".provenance-close-btn")?.addEventListener("click", () => {
      modal.style.display = "none";
    });
  }
};

const downloadProvenanceProof = (filename: string, data: any) => {
...
const copyToClipboard = (text: string, button: HTMLButtonElement) => {
  navigator.clipboard.writeText(text).then(() => {
    ...
    button.innerHTML = `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" style="vertical-align: middle; margin-right: 2px;"><polyline points="20 6 9 17 4 12"></polyline></svg> Copied!`;
    button.style.borderColor = "#00e673";
    button.style.color = "#00e673";
    setTimeout(() => {
...
const renderProvenanceModalContent = (modal: HTMLElement, story: any, tlsProof: any, summaryProof: any) => {
  const renderTabContent = (tab: string) => {
    const tabBody = modal.querySelector(".provenance-tab-body");
    if (!tabBody) return;
    ...
    if (tab === "tls") {
      tabBody.innerHTML = `
        <div class="provenance-meta-grid">
          <div class="provenance-meta-item">
            ...
            <span class="meta-value protocol-tag">${escapeHtml(tlsProof.proof_type)} ${escapeHtml(tlsProof.version)}</span>
          </div>
          <div class="provenance-meta-item">
            <span class="meta-label">Fetch Timestamp</span>
            <span class="meta-value">${escapeHtml(new Date(tlsProof.notary.timestamp).toLocaleString())}</span>
          </div>
          <div class="provenance-meta-item">
            <span class="meta-label">Target Website</span>
            <span class="meta-value">${escapeHtml(tlsProof.handshake.server_name)}</span>
          </div>
          <div class="provenance-meta-item">
            <span class="meta-label">SSL CA Signer</span>
            <span class="meta-value">${escapeHtml(tlsProof.certificate.issued_by)}</span>
          </div>
          <div class="provenance-meta-item full-width">
            <span class="meta-label">Certificate Fingerprint (SHA-256)</span>
            <span class="meta-value code-font">${escapeHtml(tlsProof.certificate.fingerprint_sha256)}</span>
          </div>
          <div class="provenance-meta-item full-width" style="position: relative;">
            ...
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
              <span class="meta-value code-font" style="word-break: break-all; flex: 1; padding: 6px 10px; background: rgba(0,0,0,0.2); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);">${escapeHtml(tlsProof.data_hash)}</span>
              <button type="button" class="copy-hash-btn" data-hash="${escapeHtml(tlsProof.data_hash)}" style="padding: 6px 10px; font-size: 0.68rem; border: 1px solid #334155; border-radius: 6px; background: rgba(148, 163, 184, 0.08); color: #cbd5e1; cursor: pointer; white-space: nowrap; display: inline-flex; align-items: center; gap: 4px;">
      ...
      `;
      const tlsBtn = tabBody.querySelector(".download-tls-btn");
      tlsBtn?.addEventListener("click", () => {
        downloadProvenanceProof(`tls_proof_${tlsProof.handshake.server_name}.json`, tlsProof);
      });
      const copyBtn = tabBody.querySelector(".copy-hash-btn") as HTMLButtonElement;
      copyBtn?.addEventListener("click", (e) => {
        e.stopPropagation();
        copyToClipboard(copyBtn.dataset.hash || "", copyBtn);
      });
    } else if (tab === "ai") {
      if (!summaryProof) {
        tabBody.innerHTML = `
      ...
      tabBody.innerHTML = `
        <div class="provenance-meta-grid">
          <div class="provenance-meta-item">
            ...
            <span class="meta-value protocol-tag ai-tag">${escapeHtml(summaryProof.model)}</span>
          </div>
          <div class="provenance-meta-item">
          ...
          <div class="provenance-meta-item">
            ...
            <span class="meta-value">${escapeHtml(summaryProof.endpoint)}</span>
          </div>
          <div class="provenance-meta-item">
            <span class="meta-label">Provider Account</span>
            <span class="meta-value code-font" style="font-size: 0.8rem;">${escapeHtml(summaryProof.providerAddress)}</span>
          </div>
          <div class="provenance-meta-item full-width" style="position: relative;">
            <span class="meta-label">0G Coprocessor Execution Proof (x_0g_proof)</span>
            <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
              <span class="meta-value code-font" style="word-break: break-all; flex: 1; padding: 6px 10px; background: rgba(0,0,0,0.2); border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);">${escapeHtml(summaryProof.x_0g_proof)}</span>
              <button type="button" class="copy-hash-btn" data-hash="${escapeHtml(summaryProof.x_0g_proof)}" style="padding: 6px 10px; font-size: 0.68rem; border: 1px solid #334155; border-radius: 6px; background: rgba(148, 163, 184, 0.08); color: #cbd5e1; cursor: pointer; white-space: nowrap; display: inline-flex; align-items: center; gap: 4px;">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                Copy
              </button>
            </div>
      ...
      aiBtn?.addEventListener("click", () => {
        downloadProvenanceProof(`ai_inference_proof_${summaryProof.responseId}.json`, summaryProof);
      });
      const copyBtn = tabBody.querySelector(".copy-hash-btn") as HTMLButtonElement;
      copyBtn?.addEventListener("click", (e) => {
        e.stopPropagation();
      ...
      const rawPayload = {
        source_tls_proof: tlsProof,
        ai_inference_proof: summaryProof
      };
      tabBody.innerHTML = `
        <div class="provenance-json-viewer">
          <pre><code>${escapeHtml(JSON.stringify(rawPayload, null, 2))}</code></pre>
        </div>
      `;
    }
  };
    ...
    <div class="provenance-modal-card">
      <div class="provenance-modal-header">
        <div>
          ...
          <span style="font-size: 0.76rem; color: #69728a; word-break: break-all;">${escapeHtml(story?.headline || "Story Verification")}</span>
        </div>
        <button type="button" class="provenance-close-btn">&times;</button>
      </div>
  ...
  // Tab handlers
  const tabs = modal.querySelectorAll(".provenance-tab-btn");
  tabs.forEach((tabBtn) => {
    tabBtn.addEventListener("click", (e) => {
      tabs.forEach((btn) => btn.classList.remove("active"));
      const btn = e.currentTarget as HTMLButtonElement;
      btn.classList.add("active");
      renderTabContent(btn.dataset.provenanceTab || "tls");
    });
  });

  // Render initial tab content
  renderTabContent("tls");
};
